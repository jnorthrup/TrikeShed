...
fun Panels.openFeedback() {
    val coords = obj()
    coords.panel = str(byId("panelName").value).ifEmpty { "(unsaved)" }
    coords.nodes = nodes().size; coords.types = nodes().map { str(it.type) }.distinct().toTypedArray()
    coords.wires = arr(graph.wires).size; coords.status = str(byId("status").textContent ?: "").take(120)
...
suspend fun Panels.onDrop(e: dynamic) {
    ...
    fun say(t: String) { val s = byId("status"); if (s != null) s.textContent = t }
...
/** the PALETTE: docked drag-source for the served vocabulary + the gallery */
suspend fun Panels.buildPalette() {
    val pal = byId("palette"); pal.innerHTML = ""
    ...
    try {
        ...
        progs.addAll(arr(pr.presets).map { str(it.name) }); progs.addAll(arr(st.panels).map { str(it.name) })
    ...
    fun render(f: String) {
        ...
        if (progs.isNotEmpty()) {
            val h = document.createElement("h5").asDynamic(); h.textContent = "gallery — programs"; body.appendChild(h)
            for (nm in progs) {
                ...
                on(d, "click", { window.location.href = "/panels?load=" + encodeURIComponent(nm) })
                body.appendChild(d)
            }
        }
    }
...
suspend fun Panels.openKeyMuxDlg() {
...
suspend fun Panels.openGallery() {
    val existing = byId("gallery")
...
fun Panels.refreshRdf() {
    if (byId("rdfPanel") == null) return
    window.clearTimeout(rdfTimer)
    rdfTimer = window.setTimeout({ val t = str(byId("rdfPanel")?.dataset?.tab ?: "graph").ifEmpty { "graph" }; launchJs { rdfTab(t) } }, 400)
}

private fun jsonPost(body: dynamic): dynamic = jsonInit("POST", JSON.stringify(body))

suspend fun Panels.rdfTab(tab: String) {
    val g = byId("rdfPanel") ?: return; g.dataset.tab = tab
...
fun Panels.openRdf() {
    val existing = byId("rdfPanel"); if (existing != null) { existing.remove(); return }
