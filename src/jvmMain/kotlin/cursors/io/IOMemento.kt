...
val xInsertString: (ByteBuffer, String?) -> Unit = { a: ByteBuffer, b: String? ->
...
val dateMapper: (String) -> LocalDate = { s: String ->
    s.let {
        val res = try {
            LocalDate.parse(it)
...
val instantMapper: (String) -> Instant = { s: String ->
    s.let {
        val res = try {
            Instant.parse(it)
        } catch (e: RuntimeException) {
...
enum class IOMemento(override val networkSize: Int? = null) : TypeMemento {
    ...
    companion object {
        ...
        fun cmp(t: TypeMemento): (Any?, Any?) -> Int = cmpMap[t] ?: { o1: Any?, o2: Any? ->
            o1.toString().compareTo(o2.toString())
        }

        fun listComparator(progression: Series<out TypeMemento>): java.util.Comparator<List<*>> = Comparator<List<*>> { o1, o2 ->
...
fun floatFillNa(fill: Float): (Any?) -> Any? {
    ...
    @Suppress("UNCHECKED_CAST") val bound: (Any?) -> Any? = { testMe: Any? ->
        (testMe as? Series<*>)?.α(x) ?: (testMe as? Iterable<*>)?.α(x) ?: (testMe as? Array<*>)?.α(x) ?: when (testMe) {
