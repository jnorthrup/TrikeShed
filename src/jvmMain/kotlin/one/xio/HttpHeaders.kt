...
enum class HttpHeaders {
    ...
    private val token: ByteBuffer? = StandardCharsets.UTF_8.encode(header)
    private var tokenLen: Int = token.limit()
    private val sendBufferSize: Int = 0

    fun getHeader(): String? {
        return header
    }

    fun getToken(): ByteBuffer? {
        return token
    }
    ...
    fun recognize(buffer: ByteBuffer): Boolean {
        ...
        if ((buffer.get(tokenLen + i) and 0xff) === ':') {
            var j: Int = 0
    ...
    companion object {
        ...
        fun getHeaders(headers: ByteBuffer): Map<String?, IntArray?>? {
            headers.rewind()
            val l: Int = headers.limit()
            val linkedHashMap: Map<String?, IntArray?> = TreeMap()
            while (headers.hasRemaining() && '\n' != headers.get());
            while (headers.hasRemaining()) {
                val p1: Int = headers.position()
                while (headers.hasRemaining() && ':' != headers.get());
                val p2: Int = headers.position()
                while (headers.hasRemaining() && '\n' != headers.get());
                val p3: Int = headers.position()

                val key: String? = StandardCharsets.UTF_8.decode(
                    headers.position(p1).limit(p2 - 1) as ByteBuffer?
                ).toString()
                    .trim()
                if (key.length() > 0) {
                    linkedHashMap.put(key, intArrayOf(p2, p3))
                }
