...
import borg.trikeshed.userspace.reactor.*
...
/**
 * KeyMux Daemon — standalone CCEK reactor element for key+quota+modelmux state.
 * 
 * Mirrors the Rust litebike keymux + reactor architecture but stays entirely
 * in TrikeShed (Kotlin/JVM, CCEK, userspace.nio). No Rust, no Swift, no FFI.
 * 
 * Components (ported from Rust litebike):
 ...
 * - ModelFacade: quota-aware model discovery + DSEL-style selection
 * - ChannelizedReactor: protocol-agnostic sequence execution with channelized dispatch
 * 
 * Runs as a standalone daemon with:
 * - UNIX domain socket health endpoint
 * - HTX reactor for KeyMux/ModelMux API calls
 * - Optional HTTP server for /api/keys, /api/models, /api/quota endpoints
 * 
 * Usage:
 *   bin/keymux-daemon --port 8888 --forge-home ~/.local/forge
 */

// ═══════════════════════════════════════════
// KeyMux Core (ported from keymux/keymux.kt)
// ═══════════════════════════════════════════
...
class KeyMux constructor(
    ...
    fun watch(prefix: String = ""): SharedFlow<Join<String, String>> = MutableSharedFlow<Join<String, String>>(replay = 0, extraBufferCapacity = 0, onBufferOverflow = kotlinx.coroutines.channels.BufferOverflow.DROP_OLDEST).asSharedFlow()
