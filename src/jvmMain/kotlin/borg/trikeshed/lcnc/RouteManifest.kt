...
object RouteManifest {
    ...
    val entries: Map<String, List<RouteEntry>> = mapOf(
        ...
        "PatchWire" to listOf(
            ...
            RouteEntry("GET", "/api/mux/catalog", "full operator catalog including provider-qualified model ids"),
            ...
            RouteEntry("GET", "/api/mux/activity", "bounded live model call receipts"),
            RouteEntry("GET", "/api/mux/sessions", "durable conversations and branch tracking"),
            RouteEntry("POST", "/api/mux/sessions", "create conversation"),
            RouteEntry("POST", "/api/mux/sessions/run", "run chat or model fan-out and fan-in"),
            RouteEntry("POST", "/api/mux/sessions/cancel", "cancel a session run and its branches"),
            RouteEntry("POST", "/api/mux/sessions/fork", "fork conversation history"),
            RouteEntry("POST", "/api/mux/sessions/update", "rename or archive conversation"),
            ...
            RouteEntry("POST", "/api/snapshots", "take a workspace snapshot {note?}: programs, prompts, project sequences, latest receipts, as one CAS citizen"),
            RouteEntry("GET", "/api/snapshots", "the snapshot lineage, newest first"),
        ...
        "KanbanModule" to listOf(
            ...
            RouteEntry("POST", "/api/archives/import", "ZIP entries (JSON {path, mediaType, base64}) into the TreeDoc/CAS pipeline; returns the manifest cid"),
            RouteEntry("GET", "/api/archives/manifest", "an archive's immutable entry identities and ordinals, by cid"),
            RouteEntry("GET", "/api/archives/content", "one restored entry's bytes, CID-verified, by cid and entry"),
            ...
            RouteEntry("GET", "/api/lcnc/contracts", "late-bound LCNC vocabulary: contracts + composites, kind hierarchy, bindings"),
        ...
        "GraalWire" to listOf(
            ...
            RouteEntry("GET", "/api/graal/allocations", "bounded recent allocation sites"),
        ...
        "HermesWire" to listOf(
            ...
            RouteEntry("GET", "/api/sleeves", "polyglot agent sleeve roll-up"),
            RouteEntry("GET", "/api/sleeves/cards", "TDD-red cards generated by unresolved traps"),
            RouteEntry("GET", "/api/sleeves/{id}", "one sleeve manifest and its traps"),
        ),
