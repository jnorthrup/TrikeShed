...
actual object PlatformCodec {
    ...
    actual val readShort = { value: ByteArray -> ((value[0].toInt() shl 8) or (value[1].toInt()
            and 0xff)).toShort() }
    actual val readInt =
        { value: ByteArray -> (value[0].toInt() shl 24) or (value[1].toInt() and 0xff shl 16) or (value[2].toInt()
                and 0xff shl 8) or (value[3].toInt() and 0xff) }
    actual val readLong =
        { value: ByteArray ->
            (value[0].toLong() shl 56) or (value[1].toLong() and 0xff shl 48) or (value[2].toLong()
                    and 0xff shl 40) or (value[3].toLong() and 0xff shl 32) or (value[4].toLong() and 0xff shl 24) or
                    (value[5].toLong() and 0xff shl 16) or (value[6].toLong() and 0xff shl 8) or (value[7].toLong()
                    and 0xff)
        }
    ...
    actual val writeBool = { value: Any? -> ByteArray(1).apply { this[0] = if (value as
                Boolean) 1
    else 0 } }
    ...
    actual val writeDouble = { value: Any? -> writeLong(java.lang.Double.doubleToLongBits(value as
            Double)) }
    ...
    actual fun createDecoder(
        ...
        // we must call the marshalling functions inside the NetworkOrder ByteArray extension functions to
        // ensure this
