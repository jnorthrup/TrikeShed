...
fun Teleported.Companion.ofGuestOrHost(o: Any?): Teleported = when (o) {
    ...
    is Map<*, *> -> Teleported.Obj(o.entries.map { it.key.toString() to ofGuestOrHost(it.value) }.sortedBy { it.first }.toMap()) // ⚡ Bolt: Use .map to prevent intermediate LinkedHashMap and ArrayList allocations
    else -> ofHost(o)
}
