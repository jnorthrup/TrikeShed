...
fun Teleported.Companion.ofGuestOrHost(o: Any?): Teleported = when (o) {
    ...
    is Map<*, *> -> Teleported.Obj(o.entries.map { it.key.toString() to ofGuestOrHost(it.value) }.sortedBy { it.first }.toMap()) // ⚡ Bolt: Prevent intermediate Map and List allocations when sorting maps
    else -> ofHost(o)
}
