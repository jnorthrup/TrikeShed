...
fun Teleported.Companion.ofGuestOrHost(o: Any?): Teleported = when (o) {
    ...
    is Map<*, *> -> Teleported.Obj(o.entries.map { it.key.toString() to ofGuestOrHost(it.value) }.sortedBy { it.first }.toMap()) // ⚡ Bolt: Prevented intermediate List and Map allocations by mapping directly to Pairs
    else -> ofHost(o)
}
