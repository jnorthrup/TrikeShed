...
object CorenlpReteCli {
    ...
    fun main(args: Array<String>) {
        ...
        CoreNlpRuntime().use { nlp ->
            for ((source, line) in lines.withIndex()) {
                ...
                for (a in axioms) {
                    ...
                    val slot = ledger.observe(packInts(cls, term), source, a.rule.evidence)
                    println("[nal] ${name(cls)} ==> ${terms[term]}  c=${"%.3f".format(Nal.truthOf(ledger.evidence(slot)).confidence)}  ← #$source $line")
                }
            }
        }
        ...
        val eternal = ledger.eternal(promote, minFrequency = 0.5f)
        val rete = ClassRete(LongArray(eternal.size) { ledger.key(eternal[it]) }, LongArray(eternal.size) { ledger.evidence(eternal[it]).packed })
        println("[rete] promoted ${rete.size}/${ledger.size} at c>=$promote, f>0.5: " +
            (0 until rete.size).joinToString { "${name(rete.antecedent(it))}==>${terms[rete.consequent(it)]}" })
        ...
        for (ask in asks) {
            ...
            val fired = rete.fire(sumo.mask(ask, SumoMask.ANCESTORS) or RoaringSeries.singleton(self))
            ...
            for (r in fired) {
                ...
                val deduced = if (rete.antecedent(r) == self) rete.evidence(r) else Nal.deduce(Nal.truthOf(rete.evidence(r)), isA)
                val ruleBasis = ledger.basis(ledger.slot(rete.key(r)))
                val direct = ledger.slot(packInts(self, rete.consequent(r)))
                val belief = when {
                    direct < 0 -> deduced
                    ledger.basis(direct).intersects(ruleBasis) -> ledger.evidence(direct)
                    else -> revise(ledger.evidence(direct), deduced)
                }
                println("[ask] $ask ==> ${terms[rete.consequent(r)]}  via ${name(rete.antecedent(r))}  deduced ${show(deduced)}" +
                    (if (direct >= 0) "  observed ${show(ledger.evidence(direct))}" else "") + "  belief ${show(belief)}  (${us}µs, model calls 0)")
            }
        }
