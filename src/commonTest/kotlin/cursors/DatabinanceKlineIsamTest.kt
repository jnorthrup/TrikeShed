...
value class KLine(val data: Vect0r<Any>) {
    ...
    inline val low: Double get() = data[KLineColumn.LOW.ordinal] as Double
...
class ColumnarKLineStore {
    ...
    fun getCursor(): Cursor {
        if (columns.isEmpty() || columns.values.any { it.isEmpty() }) {
            return SimpleCursor(
                Vect0r(0) { Scalar(IOMemento.IoNothing, "empty_kline_store") },
                Vect0r(0) { Vect0r(0) { "" } })
        }
...
class DatabinanceKlineIsamTest {
    ...
    @Test
    fun `should process KLine CSV data into columnar store, write to ISAM, read back, and verify`() {
        val csvData = listOf(
        ...
        val originalCursor = store.getCursor()

        // Test basic cursor properties
        assertEquals(3, originalCursor.size, "Original cursor should have 3 rows")
        assertEquals(
            KLineColumn.entries.size,
            originalCursor.scalars.size,
            "Original cursor should have ${KLineColumn.entries.size} columns"
        )

        // Write to ISAM and read back
        var tempIsamFile: Path? = null
        var tempMetaFile: Path? = null
        try {
            tempIsamFile = Files.createTempFile("klineIsamTest", ".isam")
            tempMetaFile = Paths.get("$tempIsamFile.meta")

            originalCursor.writeISAM(tempIsamFile.toString())
            assertTrue(Files.exists(tempIsamFile), "ISAM file should be created")
            assertTrue(Files.exists(tempMetaFile), "ISAM meta file should be created")

            FileChannel.open(tempIsamFile, StandardOpenOption.READ).use { fc ->
                val isamCursor = ISAMCursor(tempIsamFile!!, fc, tempMetaFile!!)

                // Verify ISAM cursor properties
                assertEquals(3, isamCursor.size, "ISAM cursor should have 3 rows")
                assertEquals(
                    KLineColumn.entries.size,
                    isamCursor.scalars.size,
                    "ISAM cursor should have ${KLineColumn.entries.size} columns"
                )

                // Verify first row, close price from ISAM
                val firstRowVecIsam = isamCursor.at(0)
                val closePriceFirstRowIsam = firstRowVecIsam[KLineColumn.CLOSE.ordinal].first
                assertEquals(0.11, closePriceFirstRowIsam as Double, "Close price of first KLine from ISAM")

                // Verify second row, open time via KLine value class accessor from ISAM
                val klineDataSecondRowIsam = KLine(isamCursor.at(1).left)
                val openTimeSecondRowIsam = klineDataSecondRowIsam.openTime
                assertEquals(1672531260000L, openTimeSecondRowIsam, "Open time of second KLine from ISAM")
                assertTrue(klineDataSecondRowIsam.isBullish, "Second KLine from ISAM should be bullish")

                // Verify third row, volume from ISAM
                val volumeThirdRowIsam = (isamCursor.at(2)[KLineColumn.VOLUME.ordinal].first as Double)
                assertEquals(1200.0, volumeThirdRowIsam, "Volume of third KLine from ISAM")
                assertEquals(0.02, isamCursor.kline(2).range, 1e-9, "Range of third KLine from ISAM")

                // Verify scalar metadata from ISAM
                assertEquals("volume", isamCursor.scalars[KLineColumn.VOLUME.ordinal].second)
                assertEquals(IOMemento.IoDouble, isamCursor.scalars[KLineColumn.VOLUME.ordinal].first)
                assertEquals("numberOfTrades", isamCursor.scalars[KLineColumn.NUMBER_OF_TRADES.ordinal].second)
                assertEquals(IOMemento.IoInt, isamCursor.scalars[KLineColumn.NUMBER_OF_TRADES.ordinal].first)

                // Demonstrate DSEL extensions on ISAMCursor
                assertEquals(0.1, isamCursor.opens[0], "DSEL (ISAM): First open price")
                assertEquals(0.14, isamCursor.closes[1], "DSEL (ISAM): Second close price")
                assertEquals(10, isamCursor.numTrades[0], "DSEL (ISAM): Number of trades in first KLine")
                assertEquals(
                    1672531200000L,
                    isamCursor.kline(0).openTime,
                    "DSEL (ISAM): Open time of first KLine object"
                )

                val highVolumeKlinesIsam = isamCursor.filterKLines { it.volume > 1000.0 }
                assertEquals(2, highVolumeKlinesIsam.size, "DSEL (ISAM): Should be 2 KLines with volume > 1000")
                assertEquals(0.14, highVolumeKlinesIsam.kline(0).close, "DSEL (ISAM): Close of first high-volume KLine")
                assertEquals(
                    0.13,
                    highVolumeKlinesIsam.kline(1).close,
                    "DSEL (ISAM): Close of second high-volume KLine"
                )
            }
        } finally {
            tempIsamFile?.deleteIfExists()
            tempMetaFile?.deleteIfExists()
        }
    }
}
