...
class FunctionalUringFacadeTest {
    private class ReorderedBackend : UserspaceChannelBackend {
        override val capabilities: Long = UringOp.caps(UringOp.NOP, UringOp.READ, UringOp.WRITE, UringOp.FTRUNCATE, UringOp.MADVISE, UringOp.FADVISE)
    ...
    @Test
    fun advice_arguments_are_distinct_from_submission_flags() = runTest {
        val backend = ReorderedBackend()
        val facade = FunctionalUringFacade(2, backend)
        val submission = UringSubmission(UringOp.MADVISE, -1, 4096, 4096, 0,
            userData = 304, operationFlags = 3)
        val result = facade.batchEnqueue(arrayOf(submission).toSeries()).toList()
        assertEquals(listOf(UringCompletion(304, 4096, 0)), result)
        assertEquals(listOf(submission), backend.admitted)
        assertEquals(0, backend.admitted.single().flags)
        assertEquals(3, backend.admitted.single().operationFlags)
    }
