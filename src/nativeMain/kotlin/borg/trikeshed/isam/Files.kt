...
actual object Files {
    ...
    actual fun write(filename: String, bytes: ByteArray) = PosixFile.writeBytes(filename, bytes)
    actual fun write(filename: String, lines: List<String>) = PosixFile.writeLines(  filename, lines)
    actual fun write(filename: String, string: String) = PosixFile.writeString(filename, string)
}
