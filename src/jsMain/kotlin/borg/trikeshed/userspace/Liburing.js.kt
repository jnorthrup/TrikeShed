 ...
 *
 * The backend call is where a real ring is chosen when the host has one -- on Node, `discoverNodeUringBackend` probes for real io_uring, so a Node process on Linux
 * runs baremetal against the kernel ring and a browser gets the in-memory emulation, at the same
 * call and with no branch in the caller -- this file does
