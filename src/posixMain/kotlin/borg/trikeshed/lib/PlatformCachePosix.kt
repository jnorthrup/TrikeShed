...
private fun readSysFsCacheFile(suffix: CharSequence): CharSequence? { val p = "/sys/devices/system/cpu/cpu0/cache/$suffix"; val fp = fopen(p.toString(), "r") ?: return null; val b = memScoped { allocArray<ByteVar>(256) }; val r = fgets(b, 256, fp); fclose(fp); return r?.toKString()?.trim() }
private fun readFromSysFs(): CacheTopology? {
    ...
    for (i in 0..3) { val p = "index$i"; val t = readSysFsCacheFile("$p/type")?.trim() ?: break; val l = readSysFsCacheFile("$p/level")?.toString()?.trim()?.toIntOrNull() ?: break; val s = readSysFsCacheFile("$p/size")?.trim()?.readableUnitsToNumber()?.toLong(); if (line == null) line = readSysFsCacheFile("$p/coherency_line_size")?.toString()?.trim()?.toIntOrNull(); when { l == 1 && t == "Data" -> l1d = s; l == 1 && t == "Instruction" -> l1i = s; l == 2 -> l2 = s; l == 3 -> l3 = s } }
