...
private class PosixUserspaceChannelBackend : UserspaceChannelBackend {
    override fun read(file: FileImpl, buffer: ByteBuffer, offset: Long): Int {
        ...
        val result = bytes.usePinned { pinned ->
            read(file.id, pinned.addressOf(start), length.toULong()).toInt()
        }
    ...
    override fun write(file: FileImpl, buffer: ByteBuffer, offset: Long): Int {
        ...
        if (length <= 0) return 0
        val result = bytes.usePinned { pinned ->
            write(file.id, pinned.addressOf(start), length.toULong()).toInt()
        }
        if (result > 0) buffer.position(buffer.position() + result)
        return result
    }
    ...
    override fun close(file: FileImpl): Int = if (file.id >= 0) close(file.id) else -1
}

internal actual fun openUserspaceChannelBackend(entries: Int): UserspaceChannelBackend = PosixUserspaceChannelBackend()
