...
class JsonPathTest {
    ...
    fun `test the simplest path and some misc whitespace`() {
        run {
            ...
            val path: JsPath = _root_ide_package_.borg.trikeshed.common.collections._l[0].toJsPath
            val expected = 0.0
    ...
    @Test fun test00() { val src = ("""{"0":0}""").toSeries()
        val result = jsPath(index(src) j src, _root_ide_package_.borg.trikeshed.common.collections._l[0].toJsPath, true, mutableListOf())
    ...
    fun test1() {
        ...
        val element = index(src)
        val path: JsPath = _root_ide_package_.borg.trikeshed.common.collections._l["a", "b", 2].toJsPath
        val result = jsPath(element j src, path, true, mutableListOf())
        val expected = 3.0
        assertEquals(expected, result)
    ...
    fun test2() {
        ...
        val element = index(src)
        val path: JsPath = _root_ide_package_.borg.trikeshed.common.collections._l[("a"), ("b"), (4), 0, (1)].toJsPath
        val result = jsPath(element j src, path, true, mutableListOf())
        val expected = 3.0
        assertEquals(expected, result)
    }
    ...
    fun `handle empty jsarray`() {
        ...
        val element = index(src)
        val path: JsPath = _root_ide_package_.borg.trikeshed.common.collections._l[3,0,0].toJsPath
        val result = jsPath(element j src, path, true, mutableListOf())
        if (result is Join<*,*>) {
            val r = result
    ...
    fun `test body 0 name`() {
        ...
        val element = index(src)
        val path: JsPath = _root_ide_package_.borg.trikeshed.common.collections._l["bodies", 0, "name"].toJsPath
        val result = jsPath(element j src, path, true, mutableListOf())
