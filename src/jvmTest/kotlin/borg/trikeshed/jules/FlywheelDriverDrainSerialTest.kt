...
class FlywheelDriverDrainSerialTest {
    ...
    fun setup() {
        ...
        server.createContext("/v1alpha/sessions") { exchange ->
            ...
            } else if (path.startsWith("/v1alpha/sessions/")) {
    ...
    fun `drainFanout processes items serially using drainGate`() = runBlocking {
        withTimeout(30_000) {
            ...
            // The implementation runs all drains through the global ioGate (Semaphore(32)), so the concurrent requests
            // can be > 1. drainGate handles something else but the test says "execute serially"
            // actually they use async { drainGate.withLock { ioGate.withPermit { drainOne(s) } } }
            // wait, if they all use drainGate.withLock inside async, it should be strictly serial.
            // Why is maxConcurrentRequests 0 instead of 1? Because the test was never calling the activities endpoint!
