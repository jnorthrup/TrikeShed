 ...
 */
fun t_create_buffers(buf_num: size_t, buf_size: size_t): CArrayPointer<iovec> = nativeHeap.run {
    val vec = allocArray<iovec>(buf_num.toInt())

    memScoped {
        val memptr = alloc<COpaquePointerVar>()
        repeat(buf_num.toInt()) { i ->
            t_posix_memalign(memptr.ptr, buf_size, buf_size)
            vec[i].iov_base = memptr.value
