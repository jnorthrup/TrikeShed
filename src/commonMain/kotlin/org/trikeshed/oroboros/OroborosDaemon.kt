...
// see PRELOAD.md for additional ques
...
object OroborosDaemon {
    ...
    private fun healthSocket(ops: ChannelOperations, healthSock: File): Int {
        ...
        // The socket lifecycle is the CCEK reactor's, not raw JDK NIO:
        // AF_UNIX socket → bind → listen all through ChannelOperations.
        var fd = -1
        var bindAttempt = 0
        while (fd < 0 && bindAttempt < 3) {
            val candidate = ops.socket(SocketDomain.AF_UNIX.posix, SocketType.SOCK_STREAM.mask, 0)
    ...
    private suspend fun kotlinx.coroutines.CoroutineScope.mainImpl(
        ...
        // is owned by ChannelOperations, so the supervisor (which registers the
        // platform provider) must be open BEFORE health binds — health is the
        // first IO the reactor takes.
        ...
        val healthFd = healthSocket(channelOps, healthSock)
        if (healthFd < 0) return

        val healthJob = launch(Dispatchers.IO) {
            val handle = channelOps.openChannel(64)
            while (isActive) {
                var clientFd = -1
                try {
                    clientFd = channelOps.accept(healthFd)
                    if (clientFd < 0) {
                        delay(100)
                        continue
                    }
                    ...
                    // The ALIVE/METRICS payload is delivered through the uring
                    // facade (writev → submit → CQE), not a raw channel.
                    val metricsBytes = (aliveLine + metricsJson).toByteArray()
                    var sent = 0
                    while (sent < metricsBytes.size) {
                        val slice = ByteBuffer(metricsBytes, sent, metricsBytes.size - sent)
                        if (handle.writev(clientFd, slice, 0L) < 0) break
                        val cqe = handle.wait(1)
                        val wr = cqe.firstOrNull()?.res ?: -1
                        if (wr <= 0) break
                        sent += wr
                    }
                } catch (e: Exception) { /* ignore */ }
                finally {
                    if (clientFd >= 0) runCatching { channelOps.close(clientFd) }
                }
            }
        }
        ...
        // supervisor (opened above with the health bind) auto-registers
        // ChannelOperations + JvmTlsCodecBackend + HtxReactorElement
        // via platformNioProviders().
        ...
        try {
            ...
            withContext(NonCancellable) {
                reactiveJob.cancelAndJoin()
                healthJob.cancel()
                runCatching { channelOps.close(healthFd) }
