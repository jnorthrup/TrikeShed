...
data class LocalAffine(
    ...
    val rank: Int get() = if (weightLowRankU.size > 0) weightLowRankU[0].size else 0

    fun apply(z: Series<Double>): Series<Double> =
        transformDim.j { i ->
            var acc = bias[i]
            acc += weightDiag[i] * z[i]
            (0 until rank).forEach { r ->
                var vDotZ = 0.0
                for (t in 0 until transformDim) vDotZ += weightLowRankV[t][r] * z[t]
...
// `List<T>.toSeries()` and its MutableList twin lived here on the deleted branch; master
// provides them in borg.trikeshed.lib.Series, so the duplicates are dropped, not shadowed.
