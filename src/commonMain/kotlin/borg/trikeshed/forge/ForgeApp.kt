...
private fun defaultForgeAppState(): Map<String, Any?> {
    val userId = "jim"
    val persistedJson = try {
        borg.trikeshed.common.Files.readString(borg.trikeshed.kanban.ForgeBoardPersistence.sourcePath(userId))
    } catch (e: Throwable) {
        try {
            borg.trikeshed.kanban.ForgeKanbanIngest.persistMarkdown(userId, "/tmp/hi")
            borg.trikeshed.common.Files.readString(borg.trikeshed.kanban.ForgeBoardPersistence.sourcePath(userId))
        } catch (_: Throwable) {
            val fallback = borg.trikeshed.kanban.ForgeKanbanIngest.fallbackReduction()
            borg.trikeshed.kanban.ForgeBoardPersistence.encode(
                borg.trikeshed.kanban.ForgeBoardPersistence.source(userId, fallback.source.description, "")
            )
        }
    }

    val doc = borg.trikeshed.parse.confix.confixDoc(persistedJson)
    val source = borg.trikeshed.kanban.ForgeBoardPersistence.decode(persistedJson)

    val reduction = try {
        borg.trikeshed.kanban.ForgeKanbanIngest.reduce(source)
    } catch (e: Throwable) {
        borg.trikeshed.kanban.ForgeKanbanIngest.fallbackReduction()
    }

    val board = reduction.board
    val index = borg.trikeshed.graph.CausalGraphNodeIndex()
    ...
    val entities = reduction.correlations.map { correlation ->
        ...
        borg.trikeshed.blackboard.LcncEntitySurface(
    ...
    val surface = borg.trikeshed.blackboard.BlackboardSurface.project(board.id.value, index, entities)

    val reactorCore = borg.trikeshed.userspace.reactor.KanbanFSM.current()
    val recentTaxonomy = if (reactorCore.recentTaxonomyNodes.isNotEmpty()) {
        reactorCore.recentTaxonomyNodes.takeLast(6)
    } else {
        board.cards.takeLast(6).map { it.title }
    }
    ...
    return linkedMapOf(
        "title" to board.name,
        "pageNotes" to source.description,
        "selectedItemId" to board.cards.firstOrNull()?.id?.value,
        "reactor" to linkedMapOf(
            "taxonomyNodeCount" to maxOf(reactorCore.taxonomyNodeCount, board.cards.size),
            "signalFacetCount" to reactorCore.cacheHits + reactorCore.cacheMisses + reactorCore.cacheStored + reactorCore.cacheEvicted,
            "cacheStoredCount" to reactorCore.cacheStored,
            "lastEventKind" to if (reactorCore.lastEventKind != "INIT") reactorCore.lastEventKind else "SourceReduced",
            "lastEventTimestampMs" to reactorCore.lastEventTimestampMs,
            "recentTaxonomyNodes" to recentTaxonomy,
        ...
        "blackboardId" to board.id.value,
        "surfaceRows" to surface.rows.map { row ->
            linkedMapOf(
...
fun forgeAppHtml(): String {
    val baseSeed = defaultForgeAppState().toMutableMap()
