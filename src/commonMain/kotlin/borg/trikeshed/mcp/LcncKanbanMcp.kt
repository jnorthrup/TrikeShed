        ...
        mapOf(
            ...
            "inputSchema" to mapOf(
                ...
                "properties" to mapOf(
                    ...
                    "spec" to mapOf("type" to "string", "description" to "The card's acceptance spec as RFC 2119 lines, one per line: GOAL: …, MUST: …, SHOULD: …, MAY: …, OUT-OF-SCOPE: …, REVIEW: human (a person must decide), MODEL: <id>, TOKENS: <n>, AGENT: codex|opencode (a coding agent claims the card: it edits a scratch clone of the repo and its diff and transcript become receipts; absent = a chat completion), AGENT-BUDGET: <seconds> (at most 840). The plane judge closes a claimed card when every MUST is met with an evidence id that exists on the fact plane; without a MUST the default is 'name the next action citing evidence'."),
