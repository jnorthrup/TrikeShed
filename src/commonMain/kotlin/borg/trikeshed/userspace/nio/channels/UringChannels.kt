...
object UringChannels {
    fun open(entries: Int = 256, ebpfPrograms: List<UringEbpfProgram> = emptyList(), trace: UringTrace? = null): UringChannel {
        ...
        try {
            ...
            return UringChannel(FunctionalUringFacade(entries, backend, ebpfPrograms = ebpfPrograms), trace)
    ...
    fun open(scope: CoroutineScope, entries: Int = 256, ebpfPrograms: List<UringEbpfProgram> = emptyList(),
        ...
        try {
            ...
            return UringChannel(FunctionalUringFacade.create(scope, entries, backend, ebpfPrograms, containmentPolicy),
                scope.coroutineContext[UringTrace])
        } catch (failure: Throwable) {
            runCatching { raw.close() }.exceptionOrNull()?.let { failure.addSuppressed(it) }
            throw failure
        }
