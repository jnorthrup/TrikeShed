...
object X86_64Jit {
    ...
    private const val REX_W: Int = 0x48
    ...
    private fun prologue(b: ByteBuf) {
        b.push(REX_W.toByte(), 0x55.toByte()); b.push(REX_W.toByte(), 0x89.toByte(), 0xe5.toByte())
        b.push(REX_W.toByte(), 0x81.toByte(), 0xec.toByte()); b.pushInt32(STACK_SIZE + 16)
        b.push(REX_W.toByte(), 0x53.toByte()); b.push(0x41.toByte(), 0x55.toByte())
    ...
    private fun epilogue(b: ByteBuf) {
        ...
        b.push(0x41.toByte(), 0x5d.toByte()); b.push(REX_W.toByte(), 0x5b.toByte())
        b.push(REX_W.toByte(), 0x89.toByte(), 0xec.toByte()); b.push(REX_W.toByte(), 0x5d.toByte())
        b.push(0xc3.toByte())
    }

    private fun emitOne(inst: EbpfInstruction, b: ByteBuf) {
        val op = inst.opcode().toInt() and 0xFF
        val cls = op and 0x07
        val dr = inst.dstReg(); val sr = inst.srcReg()
        val off = inst.offset(); val imm = inst.imm()

        when {
            op == 0x95 -> { b.push(0x31.toByte(), 0xc0.toByte()); b.push(0xc3.toByte()) }
            op == 0x85 -> b.push(0x31.toByte(), 0xc0.toByte())
            cls == 0x04 || cls == 0x07 -> {
                val srcIsImm = (op and 0x08) != 0
                val opBase = op and 0xF0
                if (srcIsImm) {
                    b.push(REX_W.toByte(), 0x81.toByte())
                    val sub = when (opBase) {
                        0x00 -> 0; 0x10 -> 5; 0x20 -> 4; 0x30 -> 6; 0xa0 -> 6; else -> 0
                    }
                    b.push((0xc0 or sub or xreg(dr)).toByte()); b.pushInt32(imm)
                } else if (opBase == 0xb0) {
                    b.push(REX_W.toByte(), 0x89.toByte()); _modrm(b, 3, 0, sr, dr)
                } else {
                    val opcode = when (opBase) {
                        0x00 -> 0x01; 0x10 -> 0x29; 0x20 -> 0x21; 0x30 -> 0x31
                        0x40 -> 0x21; 0x50 -> 0x09; 0x60 -> 0x31; 0x70 -> 0x21
                        0x80 -> 0x31; 0x90 -> 0x31; 0xa0 -> 0x31; 0xc0 -> 0x21
                        else -> 0x01
                    }
                    b.push(REX_W.toByte(), opcode.toByte()); _modrm(b, 3, 0, sr, dr)
                }
            }
            cls == 0x05 && (op and 0xF0) == 0x00 -> {
                b.push(0xe9.toByte()); b.pushInt32(0)
            }
            op == 0xb7 -> {
                b.push(REX_W.toByte(), (0xb8 + (xreg(dr) and 0x07)).toByte()); b.pushInt64(imm.toLong())
            }
            op == 0x9d -> { b.push(REX_W.toByte(), 0x31.toByte()); _modrm(b, 3, 0, 0, 0) }
            cls == 0x60 || cls == 0x00 -> {
                if (op == (0x60 or 0x00 or 0x18)) {
                    b.push(REX_W.toByte(), (0xb8 + (xreg(dr) and 0x07)).toByte())
                    b.pushInt64(imm.toLong() and 0xFFFFFFFF)
                }
            }
            else -> {
                if (op == 0x61) {
                    b.push(REX_W.toByte(), 0x8b.toByte()); modrmDisp(b, dr, sr, off.toInt())
                } else if (op == 0x73) {
                    b.push(REX_W.toByte(), 0x89.toByte()); modrmDisp(b, sr, dr, off.toInt())
                }
            }
        }
    }
