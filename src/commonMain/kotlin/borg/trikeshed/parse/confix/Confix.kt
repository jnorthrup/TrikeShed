...
/**
 * Per-column TypeEvidence samplers — one sampler per column, stored parallel
 * to the data. Null by default so writes stay untyped until a column is read
 * and its sampler is invoked. Each entry is derived lazily on first access
 * by scanning the column's character data with [TypeEvidence.sample].
 */
var columnSamplers: Series<TypeEvidence>? = null
...
enum class Syntax {
                    ...
                    np = when (mt) {
                        ...
                        2 -> {
                            val (len, np1) = readLen(np, ai); if (len < 0) np1 else {
                                add(open, np1 + len.toInt() - 1, IOMemento.IoByteArray); np1 + len.toInt()
                            }
                        }
    ...
    CSV {
        val delim = ','
        override fun scan(src: Series<Byte>): Cursor = scanDelim(src, delim).a
        override fun recognize(first: Byte): Boolean = true  // CSV is binary-safe, detect by heuristic
    },
    TSV {
        val delim = '\t'
        override fun scan(src: Series<Byte>): Cursor = scanDelim(src, delim).a
    ...
    // shared delimiter scanner for CSV and TSV
    protected fun scanDelim(src: Series<Byte>, delim: Char): Join<Cursor, FlatIndex> {
        val n = src.size
        val chars: Series<Char> = n j { i: Int -> src[i].toInt().toChar() }
        val sOpen  = series()  // line open positions
        val sClose = series()  // line close positions
        val sTag   = ChunkedMutableSeries<IOMemento>()  // IoArray for each line
        val sDelims = ChunkedMutableSeries<ChunkedMutableSeries<Int>>()  // field delimiters per line

        fun addLine(o: Int, c: Int, delims: ChunkedMutableSeries<Int>) {
            sOpen.add(o); sClose.add(c); sTag.add(IOMemento.IoArray); sDelims.add(delims)
        }

        var lineStart = 0
        var i = 0
        while (i <= n) {
            val eol = i == n || (chars[i] == '\n') || (i < n - 1 && chars[i] == '\r' && chars[i + 1] == '\n')
            if (eol) {
                // scan line for delimiter
                val lineDelims = ChunkedMutableSeries<Int>()
                var j = lineStart
                var inQ = false
                var esc = false
                while (j < i) {
                    val c = chars[j]
                    when {
                        inQ -> { if (esc) esc = false else if (c == '"') inQ = false }
                        c == '"' -> inQ = true
                        c == '\\' && !esc -> esc = true
                        c == delim && !inQ -> lineDelims.add(j)
                    }
                    j++
                }
                addLine(lineStart, i - 1, lineDelims)
                lineStart = if (i < n && chars[i] == '\r') i + 2 else i + 1
                i++
            } else {
                i++
            }
        }
        val colCount = if (sDelims.isEmpty()) 0 else (0 until sDelims.size).maxOfOrNull { sDelims[it].size } ?: 0
        return buildTreeDelim(sOpen, sClose, sTag, sDelims, colCount)
    }
    ...
    fun buildTree(sOpen: ChunkedMutableSeries<Int>, sClose: ChunkedMutableSeries<Int>, sTag: ChunkedMutableSeries<IOMemento>): Join<Cursor, FlatIndex> {
        ...
        return (rc j { k: Int -> row(ri[k]) }) j FlatIndex(spans, tags, depths, childOf, delims = null, colCount = null)
    }

    // CSV/TSV buildTree with delimiter info
    fun buildTreeDelim(sOpen: ChunkedMutableSeries<Int>, sClose: ChunkedMutableSeries<Int>, sTag: ChunkedMutableSeries<IOMemento>, sDelims: ChunkedMutableSeries<ChunkedMutableSeries<Int>>, colCount: Int): Join<Cursor, FlatIndex> {
        val total = sOpen.size
        val spans = total j { k: Int -> sOpen[total-1-k] j sClose[total-1-k] }
        val tags  = total j { k: Int -> sTag[total-1-k] }
        val depths = total j { idx: Int -> val si=spans[idx];(0 until total).count{k:Int->k!=idx&&spans[k].a<si.a&&spans[k].b>si.b} }
        ...
        val rc = (0 until total).count { k: Int -> depths[k] == 0 }; val ri = IntArray(rc); var rx = 0
        for (i in 0 until total) if (depths[i] == 0) ri[rx++] = i
        return (rc j { k: Int -> row(ri[k]) }) j FlatIndex(spans, tags, depths, childOf, delims = sDelims, colCount = colCount)
    }
    ...
    data class FlatIndex(
        ...
        /** CSV/TSV field delimiter positions per line; null for JSON/CBOR/YAML */
        val delims: ChunkedMutableSeries<ChunkedMutableSeries<Int>>? = null,
        /** Column count for CSV/TSV; derived from max delimiter count; null for other formats */
