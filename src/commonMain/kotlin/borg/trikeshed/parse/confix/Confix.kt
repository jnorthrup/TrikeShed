...
enum class Syntax {
    ...
    val COL_META: Series<`ColumnMeta↻`> = 4 j { c: Int ->
        when (c) {
            0 -> ColumnMeta("open", IOMemento.IoInt); 1 -> ColumnMeta("close", IOMemento.IoInt); 2 -> ColumnMeta(
            "tag", IOMemento.IoObject
        ); 3 -> ColumnMeta("children", IOMemento.IoObject); else -> error("4")
        } as `ColumnMeta↻`
    }
    ...
    fun scan0(src: Series<Byte>): Join<Cursor, FlatIndex> {
        ...
        var inQ = false
        var esc = false
        fun push(o: Int, t: IOMemento) {
            stack.add(P(o, t))
        }

        fun pop(c: Int) {
            if (stack.size == 0) return
            val p = stack.removeAt(stack.size - 1); add(p.open, c, p.tag)
        }
        ...
        while (i < n) {
            ...
            when {
                inQ -> when {
                    ...
                    't' -> {
                        if (i + 3 < n && chars[i + 1] == 'r' && chars[i + 2] == 'u' && chars[i + 3] == 'e') {
                            add(i, i + 3, IOMemento.IoBoolean); i += 3
                        }
                    }

                    'f' -> {
                        if (i + 4 < n && chars[i + 1] == 'a' && chars[i + 2] == 'l' && chars[i + 3] == 's' && chars[i + 4] == 'e') {
                            add(i, i + 4, IOMemento.IoBoolean); i += 4
                        }
                    }

                    'n' -> {
                        if (i + 3 < n && chars[i + 1] == 'u' && chars[i + 2] == 'l' && chars[i + 3] == 'l') {
                            add(i, i + 3, IOMemento.IoNothing); i += 3
                        }
                    }
                    ...
                    '-', '+', in '0'..'9' -> {
                        ...
                        while (i < n) {
                            val ch =
                                chars[i]; if (ch !in '0'..'9' && ch != '.' && ch != 'e' && ch != 'E' && ch != '+' && ch != '-') break; i++
                        }
        ...
        while (stack.size > 0) {
            val p = stack.removeAt(stack.size - 1); add(p.open, n - 1, p.tag)
        }
        return buildTree(sOpen, sClose, sTag)
    }
    ...
    fun buildTree(
        sOpen: ChunkedMutableSeries<Int>, sClose: ChunkedMutableSeries<Int>, sTag: ChunkedMutableSeries<IOMemento>
    ): Join<Cursor, FlatIndex> {
        val total = sOpen.size
        val spans = total j { k: Int -> sOpen[total.dec() - k] j sClose[total - 1 - k] }
        val tags = total j { k: Int -> sTag[total.dec() - k] }
        val depths = total j { idx: Int ->
            val si = spans[idx]; (0 until total).count { k: Int -> k != idx && spans[k].a < si.a && spans[k].b > si.b }
        }
        val childOf = { idx: Int ->
            val si = spans[idx]
            val o = si.a
            val cl = si.b
            val td = depths[idx].inc()
            val b = IntArray(total)
            var ct = 0; for (k in 0 until total) {
            if (k == idx) continue
            val sk = spans[k]; if (sk.a > o && sk.b < cl && depths[k] == td) b[ct++] = k
        }
            val c = ct
            val a = b; c j { out: Int -> a[out] }
        }
        val rvC = arrayOfNulls<RowVec>(total).toSeries()
        fun row(i: Int): RowVec {
            rvC[i]?.let { return it }
            val sp = spans[i]
            val tg = tags[i]
            val dc = childOf(i)
            val cc: Cursor = dc.size j { k: Int -> row(dc[k]) }
            val rv = (4 j { c: Int ->
                when (c) {
                    0 -> (sp.a as Any?) j COL_META[0]
                    1 -> (sp.b as Any?) j COL_META[1]
                    2 -> (tg as Any?) j COL_META[2]
                    3 -> (cc as Any?) j COL_META[3]; else -> error(
                    "4"
                )
                }
            }) as RowVec; rvC[i] = rv; return rv
        }

        val rc = (0 until total).count { k: Int -> depths[k] == 0 }
        val ri = IntArray(rc)
        var rx = 0
    ...
    fun scanIndex(src: Series<Byte>): ConfixIndex {
        ...
        return n j { op: Any? ->
            @Suppress("UNCHECKED_CAST") when (op) {
                Spans -> flat.spans
                Tags -> flat.tags
                Depths -> flat.depths
                DirectChildren -> flat.childOf
                TreeCursor -> scan(src)
                KeyToChild -> ({ key: CharSequence ->
