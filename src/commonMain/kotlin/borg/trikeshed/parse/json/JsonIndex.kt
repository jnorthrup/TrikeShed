...
class JsonIndex {
    ...
    enum class OffsetEncoding(val encode: (UByte) -> UByte, val decode: (UByte) -> UByte) {
        ...
        companion object {
            fun encode(offset: UByte, ordinal: Int): UByte = entries[ordinal].encode(offset)

            fun decode(byte: UByte): Pair<UByte, Int> = (byte.toUInt() shr 6).toInt().let { ordinal ->
                Pair(entries[ordinal].decode(byte), ordinal)
            }
        }
    }
}