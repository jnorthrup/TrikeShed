...
fun HarnessPatch.setResult(n: dynamic, v: dynamic) {
 ...
 */
fun HarnessPatch.renderGroupedBoard(n: dynamic) {
...
fun HarnessPatch.renderConcentric(n: dynamic) {
    ...
    if (truthy(cur.truncated)) html += "<div class=\"kboard-status\" role=\"status\">Partial projection: " + escA(if (truthy(cur.limit)) cur.limit else "limit") + "</div>"
    ...
    if (arr(cur.rows).isEmpty()) html += "<div class=\"kboard-status\">empty sheet — " + columns.joinToString(" · ") { escA(it.name) } + "</div>"
        ...
        arr(cur.rows).take(1024).joinToString("") { row ->
            "<tr>" + arr(row).take(64).joinToString("") { cell ->
...
fun HarnessPatch.renderHeap(n: dynamic) {
...
fun HarnessPatch.renderStandings(n: dynamic) {
    var wrap = n.el.querySelector(".csheet")
    if (wrap == null) { wrap = document.createElement("div").asDynamic(); wrap.className = "csheet"; n.el.appendChild(wrap) }
...
fun HarnessPatch.renderPanelsList(n: dynamic, panels: dynamic) {
    var wrap = n.el.querySelector(".kboard")
...
fun HarnessPatch.renderProgramRef(n: dynamic) {
...
fun HarnessPatch.renderButtonNode(n: dynamic) {
...
fun HarnessPatch.renderSliderNode(n: dynamic) {
...
fun HarnessPatch.ensureMedia(n: dynamic, url: String): dynamic {
 ...
 */
suspend fun HarnessPatch.runScope(n: dynamic, i: dynamic): dynamic {
    val guard = i["when?"]
...
fun HarnessPatch.openCardModal(n: dynamic, colId: String, item: dynamic) {
    val isNew = item == null
...
fun HarnessPatch.crumbSync() {
    val el = q("#crumb")
    val stack = arr(diveStack)
    if (stack.isEmpty()) { el.style.display = "none"; el.textContent = ""; return }
    ...
    stack.forEachIndexed { i, entry ->
        ...
        val seg = document.createElement("span").asDynamic(); seg.className = "seg"; seg.textContent = entry.name
        if (i < stack.size - 1) on(seg, "click", { popTo(i + 1) })
        else seg.style.color = "var(--ink)"
        el.appendChild(seg)
    }
}

/** On the blackboard a dive selects the referenced program; its territory is already on the landscape. */
suspend fun HarnessPatch.diveInto(name: String) {
    if (window.asDynamic().Harness != null) { window.asDynamic().Harness.select(name); return }
    ...
    val data0 = serialize()
    diveStack.push(obj { it.name = name; it.data = data0 })
    crumbSync()
...
fun HarnessPatch.popTo(depth: Int) {
    if (depth >= num(diveStack.length)) return
    val target = diveStack[depth]
    diveStack = diveStack.slice(0, depth)
    crumbSync()
    panelName().value = if (depth == 0) "" else diveStack[depth - 1].name
    load(target.data)
}
