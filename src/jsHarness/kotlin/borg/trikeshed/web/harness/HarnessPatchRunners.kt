 ...
 * dead code.
 */
class HarnessRunner(val run: suspend (n: dynamic, i: dynamic) -> dynamic, var render: ((n: dynamic, out: dynamic) -> Unit)? = null)
...
internal fun harnessRunners(): MutableMap<String, HarnessRunner> {
    ...
    return mutableMapOf(
        ...
        "vm.revoke" to HarnessRunner({ _, i -> val r = api("POST", "/api/vm/" + enc(str(i.vmId)) + "/revoke", obj()); o("ok" to !truthy(r.error)) }),
        "vms.list" to HarnessRunner({ _, _ -> val r = api("GET", "/api/vm"); o("rows" to (if (truthy(r.rows)) r.rows else r)) }),
        "pytest.pure" to HarnessRunner({ n, i ->
...
(rc, buf.getvalue()[-2000:])"""
        ...
        "graal.vitals" to HarnessRunner({ _, _ -> o("json" to api("GET", "/api/graal/vitals")) }),
        "blackboard.facts" to HarnessRunner({ _, _ -> o("facts" to api("GET", "/blackboard/facts")) }),
        "blackboard.board" to HarnessRunner({ _, _ -> o("board" to api("GET", "/blackboard/board")) }),
        "blackboard.sites" to HarnessRunner({ _, _ -> o("sites" to api("GET", "/blackboard/sites")) }),
        "http.get" to HarnessRunner({ n, _ -> o("json" to api("GET", str(pp(n, "path")))) }),
        "http.post" to HarnessRunner({ n, i -> o("json" to api("POST", str(pp(n, "path")), i.body)) }),
        // board.get / board.view: no browser runner — serverRun() sends them to the daemon's unit.
        "timer" to HarnessRunner({ _, _ -> o("tick" to js("Date.now()")) }),
        "js" to HarnessRunner({ n, i -> val f = js("Function")("x", "return (" + str(pp(n, "expr")) + ")"); o("y" to f(i.x)) }),
        "pick" to HarnessRunner({ n, i ->
            var v: dynamic = i.x
            for (k in str(pp(n, "path")).split(".").filter { it.isNotEmpty() }) v = if (v == null) undefined else v[k]
            o("y" to v)
        }),
        "display" to HarnessRunner({ n, i -> p.setResult(n, i.x); obj() }),
        "gauge" to HarnessRunner({ n, i ->
            var v: dynamic = i.x
        ...
        "project.list" to HarnessRunner({ _, _ -> val r = api("GET", "/api/projects"); o("scopes" to (if (truthy(r.scopes)) r.scopes else r)) }),
        "kg.ingest" to HarnessRunner({ n, i ->
        ...
        "beliefs.introspect" to HarnessRunner({ _, _ -> o("field" to api("GET", "/api/beliefs/introspect")) }),
        "pointcut.routes" to HarnessRunner({ _, _ -> val r = api("GET", "/api/graal/pointcuts"); o("routes" to (if (truthy(r.routes)) r.routes else r)) }),
        "list.pairs" to HarnessRunner({ n, _ ->
        ...
        "list.groupBy" to HarnessRunner({ n, i ->
        ...
        "dom.board" to HarnessRunner({ n, i ->
        ...
        "panels.list" to HarnessRunner({ _, _ -> val r = api("GET", "/api/panels"); o("panels" to (if (truthy(r.panels)) r.panels else jsArray())) },
            { n, out -> p.renderPanelsList(n, if (truthy(out) && truthy(out.panels)) out.panels else jsArray()) }),
        "program.ref" to HarnessRunner({ _, _ -> obj() }, { n, _ -> p.renderProgramRef(n) }),
        // at canvas top level a formal parameter has no caller: its declared default IS the value
        "scope.in" to HarnessRunner({ n, _ -> o("value" to pp(n, "default")) }),
        "scope.out" to HarnessRunner({ n, i -> p.setResult(n, i.value); obj() }),
        ...
        "scope" to HarnessRunner({ n, i -> p.runScope(n, i) }),
        "note" to HarnessRunner({ _, _ -> obj() }),
        "text.value" to HarnessRunner({ n, _ -> o("value" to pp(n, "value")) }),
        "json.value" to HarnessRunner({ n, _ ->
        ...
        "button" to HarnessRunner({ n, _ -> val t = n._pressedAt; n._pressedAt = null; if (truthy(t)) o("press" to t) else obj() }, { n, _ -> p.renderButtonNode(n) }),
        "slider" to HarnessRunner({ n, _ -> o("value" to num(n._sval ?: pp(n, "value") ?: 0)) }, { n, _ -> p.renderSliderNode(n) }),
        ...
        "media.player" to HarnessRunner({ n, i ->
            val url = str(i.url)
        ...
        "graal.events" to HarnessRunner({ n, _ -> o("event" to (if (truthy(n._lastEvent)) n._lastEvent else null)) }),
        "vm.events" to HarnessRunner({ n, _ -> o("event" to (if (truthy(n._lastEvent)) n._lastEvent else null)) }),
        ...
        "sheet.concentric" to HarnessRunner({ n, i ->
            val idx = obj()
        ...
        "graal.heap" to HarnessRunner({ n, _ -> val r = api("GET", "/api/graal/heap"); n._heap = r; o("heap" to r) }, { n, _ -> p.renderHeap(n) }),
        "mux.standings" to HarnessRunner({ n, _ ->
 ...
 * renders the outputs.
 */
internal fun serverRun(type: String): HarnessRunner = HarnessRunner({ n, i ->
