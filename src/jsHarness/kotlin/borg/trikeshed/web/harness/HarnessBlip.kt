...
object HarnessBlip {
    ...
    fun enter(n: dynamic, e: dynamic) {
        ...
        timer = window.setTimeout({
            timer = 0
            launchJs {
                val q = "program=" + encodeURIComponent(if (p.board.name == null) "" else str(p.board.name)) + "&node=" + encodeURIComponent(str(n.id)) + "&type=" + encodeURIComponent(str(n.type))
    ...
    private fun render(n: dynamic, r: dynamic) {
        ...
        val boardName: String? = if (p.board.name == null) null else str(p.board.name)
