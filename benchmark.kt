...
fun main() {
    ...
    for (i in 0 until numNodes) {
        for (j in 0 until 10) {
            val target = (i + j) % numNodes
            graph.addEdge(i, target, (i * target).toDouble())
        }
    }

    val startNodes = setOf(0)
    val query = GraphQuery(graph, startNodes)
    val queryOptimized = GraphQueryOptimized(graph, startNodes)

    val time2 = measureTimeMillis {
        val result = query.outE { it > 100.0 }.toSet()
        println("GraphQuery result size: ${result.size}")
    }
    println("Benchmark 2 (GraphQuery.outE): $time2 ms")

    val time3 = measureTimeMillis {
        val result = queryOptimized.outE { it > 100.0 }.toSet()
        println("GraphQueryOptimized result size: ${result.size}")
    }
    println("Benchmark 3 (GraphQueryOptimized.outE): $time3 ms")
}
```