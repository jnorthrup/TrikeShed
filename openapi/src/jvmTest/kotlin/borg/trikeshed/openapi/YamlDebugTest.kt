...
class YamlDebugTest {
    ...
    fun debugKrakenYaml() {
        ...
        val result = YamlParser.reify(text) as Map<String, Any?>
...
""".trimIndent()
        ...
        @Suppress("UNCHECKED_CAST")
        val result = YamlParser.reify(problematic) as Map<String, Any?>
        println("=== PROBLEMatic YAML ===")
        println("Keys: ${result.keys}")
        println("paths present: ${result.containsKey("paths")}")
        
        @Suppress("UNCHECKED_CAST")
        val result2 = YamlParser.reify(noTrailingNewline) as Map<String, Any?>
