...
| B run block | `lcnc/LcncRunHead.kt` (commonMain, NEW: the run-head decision, pure. `canonical`/`canonicalInputs` fold a request's inputs to one text — recursive key sort plus `WorkspaceSnapshot`'s integral-Double-to-Long, because every JSON number arrives as a Double and a WAL-replayed receipt comes back the same way, so `{"n":3}` and `{"n":3.0}` are ONE target; `rows`/`head` pick the runs of `(programKey, programCid, canonical inputs)` sorted started-desc then board-sequence-desc, splitting the newest COMPLETED run (`artifact`, what the frame shows, the way make keeps the old `.o`) from the newest run of any status (`latest`, what the lamp describes); `decide` returns one of five lamps — Never built, Running, Completed, Stale, Failed — from `latest` plus `active` plus the stale marker, never from a timestamp; `movedNames` reads the marker's rows and falls back to `project + "/ listing"` for the blank id `RunStaleProduction` binds for a listing change; `headBody` is the bounded projection the read route answers with, and EVERY list in it is capped at 64 — the consumed rows, the stale marker's rows and the moved names alike, each saying so beside `shownTruncated` (`consumedTruncated`, `staleTruncated`, `movedTruncated`) while `stale.count` stays the marker's whole count, and the Stale reason names the first eight moved documents and counts the rest. Capping the marker is the load-bearing one: `LcncStaleMarker.merge` folds one row per moved document and never expires one, so a completed run over a project whose documents keep moving grew a 13-node, ~200-character row per document (two 71-character cids) until the read route's own preflight answered 413 — which a page reads as a lamp-less `refused` frame with NO button, permanently, on exactly the Stale target the cut exists to rebuild. `fitToBudget` is the belt to that pair of braces and the ladder the route sheds by INSTEAD of refusing: the shown output first (its receipt cid still reads the bytes from `/api/lcnc/content`), then the stale rows (the count survives), then the consumed rows, then the moved names, refusing only when the skeleton alone is over budget — a lamp with no output still tells the reader the truth and still offers the button that fixes it. `decide`'s overdue reason floors its elapsed seconds to a ten-second grain, so a polling page's frame does not churn once a second under the reader's cursor; it is an ANSWER envelope — `ok` and a `lamp`, the failed run's own message as `runError` beside `status` and `phase`, and NEVER a top-level `error`, which is the refusals' word alone, so no reader can mistake "this target failed" for "this route could not answer". Deliberately NOT keyed on `inputFingerprint`: that is computed over the cids a run READ and moves whenever a document moves, which is the very event a build frame must survive); `docs/RunBlock.kt` (commonMain, NEW: the fence grammar and the frame, pure. `scan` finds every `lcnc-run` fence in top-level document order and numbers them by their own count; `parse` accepts a JSON object with a non-blank `program`, an absent-or-object `inputs` and an optional String `show`, and refuses — with its own bytes escaped and NO button — a body that is not JSON, names no program, spells `inputs` as anything but an object, or carries `document` (an inline ring records a null `programKey`, so the head route could never find it and the page would grow a Build whose result it cannot show); `state` discriminates on the ENVELOPE, never on a field name: a body with a `lamp` is the verdict it carries (a Failed one included, whose `runError` is just text in the reason), and only a body with `error` and no `ok` becomes the lamp-less, `refused` "Unavailable" that offers nothing to press — reading `error` first was the defect that made every Failed frame, and every run a restart stamped `interrupted`, unbuildable from the page; `pending` is the pressing tab's optimistic Running that keeps the artifact on the page; `HEAD_PATH`/`RUN_PATH`/`REBUILD_PATH` with `headQuery`, `buildRequest` and `rebuildRequest` are the three requests a frame can make — the cut's own MUSTs, so they are decided here beside the rule that decides WHICH button to draw, and pinned on jvm and js, rather than living as literals in the browser half where no test on any target could see them; `rebuildRequest` is null for a frame with no run id, the same guard `buttonHtml` makes by drawing Build instead of Rebuild; `frameId` is the element id the page repaints in place; `frameHtml` emits ONE button — Build for Never built/Failed, Rebuild for Completed/Stale, disabled while Running, and none at all ONLY for a `refused` frame — with the lamp pill, the reason, the canonical inputs, the receipt cid shortened by `DocumentSurface.shortCid`, `rebuildOf`, the moved names and the shown output; a one-entry map holding one string renders as markdown, everything else as escaped JSON, so no display sink's output key is ever pinned); `docs/DocumentMarkdown.kt` (commonMain: `render` gains a defaulted `fence: (info, body) -> String?` seam handed the RAW body, so a caller owns a block by its info string and this renderer still knows nothing about LCNC; the handler is NOT passed into the blockquote recursion — quoting a build target is quoting it, not arming it); `docs/DocumentSurface.kt` (commonMain: `documentHtml` gains a defaulted `runs: Map<Int, RunBlock.State>` and passes the fence handler in the markdown branch only); `kanban/module/LcncRunService.kt` (jvmMain: `head(program, inputsText, show)` — the version is DERIVED from `ctx.programLoader` + `LcncBlackboard.cidOf`, byte-for-byte the cid `execute`'s freeze mints, so the answer is the version the next Build would run and is true in a bare rig with no board entry; ONE `blackboard.snapshot()` per request, no key copy; refusals `program_required`, `bad_inputs`, `inputs_must_be_object`, `no_such_program`, and a `ValueBudget` preflight that DEGRADES through `LcncRunHead.fitToBudget` and 413s only when the skeleton itself is over budget); `kanban/module/KanbanModule.kt` (jvmMain: one literal claim `GET /api/lcnc/runs`, beside `/api/lcnc/content`, query parsed by `CouchHttpSurface.parseQuery`); `lcnc/RouteManifest.kt` (one `RouteEntry` in the KanbanModule block); jsMain `docs/DocumentSurfacePage.kt` (fetch, encoding and clicks only: `RunBlock.scan` on open, one head read per block built from `RunBlock.headQuery` (the page's only say is `encodeURIComponent`), a delegated listener extended to `[data-run-build],[data-run-rebuild]`, `postJson` of `RunBlock.buildRequest`/`rebuildRequest`, and a 3-second poll cancelled by a generation counter — the poll is what makes the Stale transition visible without an event stream. WHAT A POLL MAY TOUCH: `#ds-doc` is the scroll container, so the poll compares the new `State` to the stored one (a data class: a quiet head compares equal) and repaints ONE frame by `outerHTML` when it moved, never the pane. Re-rendering three panes every three seconds threw the reader back to the top of a long page, dropped the text they were selecting, and — for a tick landing between mousedown and mouseup — replaced the button under the cursor so the click never fired; a full `render()` is now for navigation alone, and a press repaints its own frame the same way); `web/documents.html` (CSS only: the `.ds-run` frame and the lamp pills, in `web/landscape.js`'s palette so the harness and the document surface say the same thing). NO daemon wiring, NO lego, NO contract, NO blackboard key family, NO preset, NO static-asset row, NO OpenAPI row: every seam the head route needs (`blackboard`, `programLoader`, `clock`) is already on `ModuleContext` and already bound by the daemon | `LcncRunHeadTest` (15 cases, commonTest on jvm AND js: canonical inputs ignore key order and 3 vs 3.0 and round-trip as a fixpoint; only entries that name a run are rows and a WAL-replayed Double reads as Long; never built; different inputs and a different `programCid` are different targets and an inline ring is never a page's head; the board sequence breaks a frozen-clock tie; a failed run keeps the last good artifact visible and Rebuild names the artifact; every terminal status maps to one of the five words; an overlapping active run wins the lamp; an overdue active run stays Running and says so; a stale marker built through `LcncStaleMarker.merge` from `RunStaleProduction`'s own bindings reads Stale naming `a.md`, and a blank id reads `genesis-notes/ listing`; the body is bounded, says `shownMissing` for a node that never ran and `shownTruncated` for an output too large, and carries the verdict, the stale rows and the rebuild lineage; a marker built through `LcncStaleMarker.merge` with 700 moved documents over a run that consumed 700 — the shape the rule really writes — still mints a body with NO `ValueBudget` violation, the `stale` count 700 beside 64 rows, all four truncation flags set, the artifact still painting, and nothing left for `fitToBudget` to shed; and the shed ladder itself, pinned by node arithmetic rather than by guesswork — budgets derived from the fixture's own counts prove shown goes first, then the stale rows (the count surviving), then the consumed rows, then the moved names, that every shed body still carries its lamp and its run id, and that only a skeleton over budget is refused); `RunBlockTest` (15 cases, commonTest on jvm AND js: the fence is found and its body is the request verbatim; only `lcnc-run` fences take an ordinal and a quoted one is not a target; the renderer hands the handler the RAW body and leaves unclaimed fences literal; seven refusals render escaped bytes with no button; the route's answer becomes the state and a `no_such_program` answer offers nothing to press; ONE button per lamp and a disabled one before the daemon has answered; the frame shows the shortened receipt cid, `rebuildOf`, the moved names and the shown prose as markdown; `shownMissing`/`shownTruncated` are said, not hidden; the pressing tab's optimistic Running keeps the artifact; and — the boundary the rest of the suite straddled — the bytes `headBody` actually mints, round-tripped through the one stringifier and parser, become a frame with the right lamp and EXACTLY ONE button for all five lamps, every failure status the runner can write (`failed`, `refused`, `timed_out`, `cancelled`, `interrupted`) keeps its Build button with the last good artifact still painting, and only the four refusal envelopes take the button away; a 700-document marker still paints exactly one Rebuild button with the moved line saying `and 636 more`; a quiet head reads back as an EQUAL `State` — including a stuck Running one polled three seconds later, which is what lets the page repaint nothing on a quiet tick — while a head that moved does not; and the three requests a frame can make are exactly `/api/lcnc/runs?program=&inputs=[&show=]`, `/api/lcnc/run` with the fence's own bytes taken from the page, and `/api/lcnc/run/rebuild` with `{"runId":"r1"}`, null when there is no run); `DocumentMarkdownTest` gains `aFenceHandlerMayOwnABlockByItsInfoString` — the five pre-existing exact-string assertions are byte-identical, which is the proof the fence change is additive; `DocumentSurfaceTest` gains `runBlocksRenderAsFramesInTheFlowOfThePage` (a good block and a malformed one on one page, one button between them, unread frames with no invented lamp, and a run block on a non-markdown document is text); `RunHeadRouteTest` (7 cases, jvmTest on `LcncRunProgramRouteTest`'s rig: the four refusals plus 405; never built then completed with the named node's output, the returns with no `show`, and `shownMissing` for a ghost node; a different key order and 3 vs 3.0 name the same target while different inputs do not; a re-published program reads Never built again and building it makes the new receipt the head without disturbing the old; the frozen clock proves the sequence tiebreak; a failed run burns Failed with the phase and the error in the reason while the last good artifact still paints, the 200 body carries no top-level `error` at all, and the daemon's own bytes read through `RunBlock.state` paint a Failed frame with its Build button; and a `lcnc/stale/<runId>` naming 700 moved documents, folded through `LcncStaleMarker.merge` and put on the rig's blackboard, is ANSWERED 200 and not refused — the daemon's own bytes carry `stale.count` 700 with 64 rows, pass `ValueBudget` and paint a Stale frame with exactly one Rebuild, where before the cap the same request answered `413 {"error":"work_limit"}`); `CorpusStaleRebuildRouteTest` gains `theRunHeadRouteBurnsTheSameFourLampsAPageShows` (the one lamp the lighter rig cannot reach: `never_built`, then `completed` with the receipt cid and the digest naming `a.md`, then — after `uploadPut` and a tendon drain, polled to a 10 s deadline — `stale` with `moved == ["a.md"]` and the artifact still the last good build, then Rebuild flipping back to `completed` with `rebuildOf` the old receipt cid, `rebuildOfRunId` the old run id and the digest carrying `revised`). Gates green: `RouteManifestParityTest`, `LcncContractParityTest`, `LcncShakeDemoTest`, `BlackboardChangesFactElementTest`, `LcncPresetCatalogTest`, `LcncPresetsGateTest`, `PresetAssemblyTest`, `PresetRequiredInputsTest`, `KernelParityTest`, `McpSurfaceParityTest`, `LcncRunProgramRouteTest`, and the CCEK coverage gate at 0 GAPs; `compileKotlinJs` green with NOT ONE line added to `gradle/js-target-debt.excludes`, and all 40 new/changed commonTest cases green on `jsNodeTest` (DocumentMarkdownTest 6, DocumentSurfaceTest 4, RunBlockTest 15, LcncRunHeadTest 15). `ForgeHostSpecParityTest > specMatchesTheRouteTableBothWays` is RED at HEAD and untouched — `GET /blackboard/sheet` is in `BlackboardWire.ROUTES` with no yaml row, and a KanbanModule claim is not in `ForgeHostRoutes.ALL` | NOT DONE in this session. The bundle was built and staged (`stageKotlinJs`; the 976,913-byte `TrikeShed.js` carries `data-run-build`, `data-run-rebuild`, `/api/lcnc/runs` and `ds-run-lamp`, and `documents.html` carries the lamp CSS), but no daemon was launched and no browser was driven: the five states (Never built, Completed with the receipt cid, Stale naming `a.md`, the Rebuild press, the new digest with `rebuildOf`) are owed by the rendered checker who follows. Nothing above is claimed from a rendered page |
...
Cut B makes a page a build target and nothing else. The block is a request written in a
document, so whoever can write a page into a mounted project can run any program with any inputs
on this daemon — the same authority the harness already grants any browser on the operator port,
but a new path to it, and worth saying out loud rather than discovering later. The page holds no
pin: the version a block builds is whatever the daemon holds for that name right now, and
`GET /api/lcnc/runs` decides the head against it, so a re-published program reads Never built
until it is built again. Writing the page from the surface is Cut E's; `run.output` — a run's
artifact as another program's input — is Cut K's.

Cut B is also the first cut where the deferred `pages` ruling has a visible consequence.
`preset-corpus` reads `glob=*.md` as a PARAM, not an input, so a block's `inputs` cannot narrow
it: a `digest.md` uploaded into `genesis-notes` becomes a third document its own digest
summarises. That does not loop — the page's bytes never change during a Build — but it is
visible in every frame, and the rendered check should screenshot it as the ruling's evidence
rather than hide it. One ordering matters: add the page BEFORE the first Build, or the added
file moves the listing fingerprint and the very first observation is Stale-by-listing rather
than Never built. The ruling row below stays a recommendation.
...
| A page holding a block inside the glob its program reads | pages of kind `pages` are excluded from `project.docs` unless the glob names them; the alternative is a self-consuming target — **still a recommendation**: no `pages` kind lands in this run, so a page inside its own program's glob stays the author's problem. Cut B is where the consequence becomes visible: `preset-corpus` reads its glob as a PARAM, so a block's `inputs` cannot narrow it and a `digest.md` in `genesis-notes` is a third document its own digest summarises. It does not loop (the page's bytes never change during a Build) and the frame is honest about it; the ordering that matters is that the page be added BEFORE the first Build, or the added file moves the listing fingerprint and the first observation is Stale-by-listing rather than Never built |
