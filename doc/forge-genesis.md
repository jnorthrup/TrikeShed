...
- The JS test target could not compile commonTest at HEAD: 214 errors in 20 files, eleven of them
  importing the JVM-only `kotlinx.coroutines.runBlocking` and the rest depending on the repo's
  `runBlocking` expect, which had JVM, native and wasm actuals but no JS one. A JS actual that
  throws (the wasm precedent) and the eleven imports now let it compile; coroutine suites on JS
  fail loudly if selected, pure suites run. That is a compile repair, not a claim that the
  coroutine suites pass on JS.
