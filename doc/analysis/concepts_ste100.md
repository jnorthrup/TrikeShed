# TrikeShed Concept Map - ASD-STE100 Simplified Technical English

Source: doc/concepts.md (merged into README.md).
Rules: ASD-STE100 per the specification summary at
https://en.wikipedia.org/wiki/Simplified_Technical_English
Code blocks stay unchanged. Code is not prose.
...
Architecture docs.
Doc/rewire.md (user-centric Forge workspace architecture, storage unification, K8s emulation via GraalVM pointcut server), doc/taste.md (high-performance hierarchical-UI engine principles, 10-point gap review).
Compiled-out layers.
Classfile/slab/ is excluded from commonMain compile in build.gradle.kts (~20 TODO() stubs: GraalJS eval, DuckDB c-interop, FacetedCursorContract, MiniDuckContract.
...
Doc/rewire.md §0.
Projection registry (project(cid): Lens) is the one new piece (T-CAS-PROJ-1).
...
No second hand-keep field table.
...
CID-derived id/rev is build/make.
...
Each is a CCEK-role element that needs to forward through the CCEK context (suspending coroutines) instead of running on the caller's stack.
The merge path is to wrap every blocking operation in withContext(Dispatchers.IO) plus a real waitFor/timeout, then route its result through NioSupervisor / MuxReactorElement so the coroutine scheduler owns cancellation.
...
Typing / at the start of a block opens a slash menu with: Text, Heading 1/2/3, To-do, Bulleted list, Numbered list, Quote, Code, Divider.
...
Seed hydration.
The baked seed carries lcncEntities (→ bulleted list in the document + cards on the board), causalNodes (→ causal graph), and gallery (→ widget catalog).
...
1.
Vertical slice.
Failing contract test → minimal production wiring → adjacent/full check (n).
2.
...
5.
Pre-commit.
Git diff --check, check no kotlinx-serialization-json/cbor in commonMain, run focused tests.
6.
Evidence.
...
Other value in situations where sometimes a lambda might be generative but constant can be distinctly picked out from in the code
...
May warrant a seperate lock-seek-read-unlock strategy for windows, or someone with ambition to port the mmap calls (resolved by MingwIsamOperations using seek-read-write)
...
Each segment is the complete json object, array, or value including the open/close brackets to recreate the json and act as discriminators for the type of the segment.
