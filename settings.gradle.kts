...
// Note: ng-sctp, classfile (nested gradle), miniduck-memory (depends on classfile), ipfs (build errors), forge (build errors) excluded
val libsDir = rootDir.resolve("libs")
if (libsDir.exists() && libsDir.isDirectory) {
    libsDir.listFiles()!!
        .filter { it.isDirectory }
        .filter { it.name != "ng-sctp" && it.name != "classfile" && it.name != "miniduck-memory" && it.name != "forge" }
