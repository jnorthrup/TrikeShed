...
## 2024-05-24 - ProcessBuilder Environment Leak in JvmProcessOperations and ProcessWorkerJvm
**Vulnerability:** ProcessBuilder in `JvmProcessOperations.kt` and `ProcessWorkerJvm.kt` inherited the host process environment variables by default, potentially leaking daemon secrets like API keys to arbitrary spawned processes.
**Learning:** Even internal tool execution wrappers (like `ProcessOperations` and `ProcessWorker`) should strictly isolate their environments because they might be used to spawn untrusted scripts or tools that dump the environment.
**Prevention:** Always clear `ProcessBuilder.environment()` and populate it explicitly with `GuestEnvironment.curated()` before appending the specific requested environment map, closing the environment leak surface for all child processes.
