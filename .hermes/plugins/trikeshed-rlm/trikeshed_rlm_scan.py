...
# ---------------------------------------------------------------------------
def cmd_scan(root: Path, libs_dir: str, src_dir: str | None = None) -> dict[str, Any]:
    libs = Path(libs_dir)
    ...
    # If a root src/ is provided, scan it as a virtual module named "_root"
    if src_dir:
        src_path = Path(src_dir)
        if not src_path.is_absolute():
            src_path = root / src_path
        if src_path.exists():
            result = _scan_single(src_path, root, out)
            results.append(result)
...
# ---------------------------------------------------------------------------
# report: human-readable audit summary from last scan-results.json
# ---------------------------------------------------------------------------
def cmd_report(root: Path) -> None:
    scan_path = root / "build" / "trikeshed-rlm" / "scan-results.json"
    if not scan_path.exists():
        print("No scan results found. Run: scan --libs-dir libs --src-dir src")
        return
    ...
    # tally by kind across all modules
    kind_totals: dict[str, int] = {}
    module_kind_counts: dict[str, dict[str, int]] = {}
    violations_dir = root / "build" / "trikeshed-rlm"

    for r in results:
        name = r["module_name"]
        vpath = violations_dir / name / "violations.json"
        if not vpath.exists():
            continue
        ...
        if per_kind:
            module_kind_counts[name] = per_kind

    total_v = sum(kind_totals.values())
    mods_with_v = len(module_kind_counts)

    print(f"SCAN RESULTS — {len(results)} libs, {total_v} violations across {mods_with_v} modules")
    print(f"(scan timestamp: {ts})")
    print()

    for kind, total in sorted(kind_totals.items(), key=lambda x: -x[1]):
        # modules with this kind, sorted by count desc
        hits = sorted(
            [(m, c[kind]) for m, c in module_kind_counts.items() if kind in c],
            key=lambda x: -x[1],
        )
        top = ", ".join(f"{m} ({n})" for m, n in hits[:8])
        print(f"  {kind} — {total} hits in {len(hits)} modules")
        print(f"    Worst: {top}")

        # for any_leak break down by subkind
        if kind == "any_leak":
            subkind_totals: dict[str, int] = {}
            for r in results:
                name = r["module_name"]
                vpath = violations_dir / name / "violations.json"
                if not vpath.exists():
                ...
                vdata = json.loads(vpath.read_text())
                for v in vdata.get("violations", []):
                    if v.get("kind") == "any_leak":
                        sk = v.get("subkind", "unknown")
                        subkind_totals[sk] = subkind_totals.get(sk, 0) + 1
            for sk, sc in sorted(subkind_totals.items(), key=lambda x: -x[1]):
                print(f"    {sk}: {sc}")
        print()
...
def main() -> None:
    ...
    s_scan.add_argument("--src-dir", default=None,
                        help="Root kotlin src/ dir to scan as a virtual module (e.g. src)")
    ...
    dispatch = {
        "scan": lambda: cmd_scan(root, args.libs_dir, getattr(args, "src_dir", None)),
