package io.trikeshed.cursor.slab.duckdb
...
object DuckDBTypes {
    ...
    val INTEGER_ARRAY = LogicalType(100 + 4, 0)  // INTEGER[]
    val VARCHAR_ARRAY = LogicalType(100 + 12, 0) // VARCHAR[]
}

// ==================== FACET TAGS FOR DUCKDB SLABS ====================
object DuckDBFacet {
    /** Slab loaded from persistent .duckdb file */
    val PERSISTENT = SlabFacet(1L shl 8)
    /** Slab in WAL (uncheckpointed) */
    val WAL_BUFFER = SlabFacet(1L shl 9)
    /** Slab checkpointed to Parquet/Arrow */
    val COLUMNAR_EXPORT = SlabFacet(1L shl 10)
    /** Slab in-memory only */
    val EPHEMERAL = SlabFacet(1L shl 11)
    /** Slab with vectorized UDF applied */
    val COMPUTED = SlabFacet(1L shl 12)
    /** Slab with ART index attached */
    val INDEXED = SlabFacet(1L shl 13)
}
