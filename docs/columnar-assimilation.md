...
Run the repository's focused Cursor verification and synthetic benchmark under `bench/columnar` (see its README). They compile current source files and generate their own data; neither reads `/tmp/trikeshed-columnar-b163`. This source slice is separate from the root multiplatform build, so a passing run does not establish whole-repository or native/JS/Wasm build health. The root build's existing configuration blockers and the focused run's exact versions/results are reported with the benchmark evidence.
