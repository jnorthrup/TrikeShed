...
| `js.leaf-delegation` | OK | 12 | phase=DELEGATED served={GUEST=12, SHADOW=2, MEMO=9} fires=1 |
| `js.process-wall` | OK | 571 | child pid alive=true; fib(10)=Num(v=55); host.call across wall=Num(v=8) |
| `python.eval` | OK | 51 | fib(15)=Num(v=610) |
| `python.host-delegate` | OK | 38 | host.call('double',21)=Num(v=42) |
| `python.statement-limit` | BOUNDED | 0 | statementLimit unsafe on PYTHON (GIL assert) → stop=INTERRUPT |
| `python.wall-interrupt` | OK | 431 | INTERRUPTED after 401ms; alive=true |
| `python.root-observation` | OK | 56 | 177 fib frames via binding pointcuts, 177 self-contained |
| `python.leaf-delegation` | OK | 115 | phase=DELEGATED served={GUEST=12, SHADOW=2, MEMO=9} fires=1 |
| `python.process-wall` | OK | 1831 | child pid alive=true; fib(10)=Num(v=55); host.call across wall=Num(v=8) |
