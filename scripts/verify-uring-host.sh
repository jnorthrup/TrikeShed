#!/usr/bin/env bash
# Current-source JVM host/probe/selection slice. Unchanged dependencies use a recorded support build.
set -euo pipefail
...
TRIKESHED_VERIFY_OUT=${TRIKESHED_VERIFY_OUT:-$(mktemp -d "${TMPDIR:-/tmp}/trikeshed-uring-host.XXXXXX")}
...
TEST_SOURCES=(
  src/commonTest/kotlin/borg/trikeshed/userspace/UringModuleProbeTest.kt
  src/jvmTest/kotlin/borg/trikeshed/platform/HostDescriptorJvmTest.kt
  src/jvmTest/kotlin/borg/trikeshed/userspace/UringJvmDiscoveryTest.kt
)
TRIKESHED_COMPILE_CP="$TRIKESHED_SUPPORT_JAR:$TRIKESHED_COROUTINES:$TRIKESHED_SERIALIZATION"
TRIKESHED_RUNTIME_CP="$TRIKESHED_VERIFY_OUT/uring-host-prod.jar:$TRIKESHED_COMPILE_CP:$TRIKESHED_KOTLIN_LIB/kotlin-stdlib.jar"
TRIKESHED_TEST_CP="$TRIKESHED_RUNTIME_CP:$TRIKESHED_KOTLIN_LIB/kotlin-test.jar:$TRIKESHED_KOTLIN_LIB/kotlin-test-junit.jar:$TRIKESHED_JUNIT:$TRIKESHED_HAMCREST"
...
printf 'Scoped current-source host/probe/selection verification; support artifact: %s\n' "$TRIKESHED_SUPPORT_JAR"
printf 'Linux module and CQE fixtures are simulated; the runtime check uses the actual JVM host.\n'
"$TRIKESHED_KOTLINC" -jvm-target 25 -Xmulti-platform -Xcommon-sources="$TRIKESHED_COMMON_CSV" \
  -classpath "$TRIKESHED_COMPILE_CP" -d "$TRIKESHED_VERIFY_OUT/uring-host-prod.jar" \
  "${COMMON_SOURCES[@]}" "${JVM_SOURCES[@]}" > "$TRIKESHED_VERIFY_OUT/compile-prod.log" 2>&1 || {
    cat "$TRIKESHED_VERIFY_OUT/compile-prod.log"; exit 1;
  }
"$TRIKESHED_KOTLINC" -jvm-target 25 -Xfriend-paths="$TRIKESHED_VERIFY_OUT/uring-host-prod.jar" \
  -classpath "$TRIKESHED_TEST_CP" -d "$TRIKESHED_VERIFY_OUT/uring-host-tests.jar" \
