...
data class SplatCreated(val splat: Splat, override val timestampNanos: Long) : SplatEvent() {
...
data class SplatUpdated(override val splatId: Long, val changes: SplatAttributes, override val timestampNanos: Long) : SplatEvent() {
    override val eventType = SplatEventType.UPDATED
    override fun withTimestamp(ts: Long) = copy(timestampNanos = ts)
    override fun reify(flavor: ReificationFlavor): Any = TODO()
}

data class SplatCulled(override val splatId: Long, val reason: String, override val timestampNanos: Long) : SplatEvent() {
    override val eventType = SplatEventType.CULLED
    override fun withTimestamp(ts: Long) = copy(timestampNanos = ts)
    override fun reify(flavor: ReificationFlavor): Any = TODO()
}

data class SplatMotionApplied(override val splatId: Long, val delta: Series<Double>, val version: Long, override val timestampNanos: Long) : SplatEvent() {
...
data class EigenEvent(
    ...
    override val splatId: Long? = splatId
    override fun withTimestamp(ts: Long) = copy(timestampNanos = ts)
    override fun reify(flavor: ReificationFlavor): Any = TODO()
}
...
data class LossEvent(
    val splatId: Long, val lossType: LossType, val value: Double,
    val gradient: Series<Double>?, val iteration: Int, override val timestampNanos: Long
) : SplatEvent() {
...
data class LossVal(val splatId: Long, val loss: Double, val iteration: Int, override val timestampNanos: Long) : SplatEvent() {
    override val eventType = SplatEventType.LOSS_VAL
    override val splatId: Long? = splatId
    override fun withTimestamp(ts: Long) = copy(timestampNanos = ts)
    override fun reify(flavor: ReificationFlavor): Any = TODO()
