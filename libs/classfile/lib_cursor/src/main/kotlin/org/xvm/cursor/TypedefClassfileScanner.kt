 ...
 * TypedefResolutionSeries for CRMS fold analysis.
 */
class TypedefClassfileScanner {
        ...
        for (i in codeElements.indices) {
            ...
            if (element is InvokeInstruction) {
                val opcode = when (element) {
                    ...
                    is Invokeinterface -> 0x24 // NVOK_N0 variant
                ...
                if (isTypedefRelated(owner, name)) {
                    emitTypedefEvent(opcode, owner, name, desc, siteOrdinal, true)
                    siteCount++
                    siteOrdinal++
                }
            }
            ...
            if (element is New) {
                ...
                if (isTypedefRelated(owner, "<init>")) {
                    emitTypedefEvent(0x38, owner, "<init>", "()V", siteOrdinal, true)
                    siteCount++
                    siteOrdinal++
                }
            }
            ...
            if (element is FieldInstruction) {
                val opcode = when (element) {
                    is Getfield -> 0xA5  // L_GET
                    is Putfield -> 0xA6  // L_SET
                    is Getstatic -> 0xA7 // P_GET
                    is Putstatic -> 0xA8 // P_SET
                    else -> continue
                }
                ...
                if (isTypedefRelated(owner, name)) {
                    emitTypedefEvent(opcode, owner, name, element.fieldType().descriptorString(), siteOrdinal, true)
                    siteCount++
                    siteOrdinal++
                }
            }
    ...
    /** Emit a typedef event to the TypedefResolutionSeries. */
    private fun emitTypedefEvent(
        ...
        // Record to TypedefResolutionSeries
        TypedefResolutionSeries.record(poolId, siteOrdinal, className, formatName, success)
    }
    ...
    companion object {
        ...
        fun scanAndPrint(root: Path) {
            ...
            // Print TypedefResolutionSeries snapshot
            val events = TypedefResolutionSeries.snapshotEvents()
            println("\nEvents captured: ${events.a}")
            events.view.take(20).forEach { (fact: TypedefFact) ->
                println("  factId=${fact.factId} pool=${fact.poolId} site=${fact.siteOrd} class=${fact.clsName} fmt=${fact.`format`} ok=${fact.success}")
            }
        }
...
fun main(args: Array<String>) {
    if (args.isEmpty()) {
        ...
        println("  Emits events to TypedefResolutionSeries for CRMS fold analysis")
        return
    }
