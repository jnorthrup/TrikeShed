...
class ConfixDocStore(
    ...
    private fun genRev(): String = when (revPolicy) {
        RevPolicy.UUID -> java.util.UUID.randomUUID().toString()
        RevPolicy.TIMESTAMP -> "${System.currentTimeMillis()}-$seq"
        RevPolicy.SEQUENTIAL -> "$seq-${java.util.UUID.randomUUID()}"
    }
...
object ConfixDocStoreFactory {
    ...
    fun createTimestamped(): ConfixDocStore = ConfixDocStore(RevPolicy.TIMESTAMP)
    fun createSequential(): ConfixDocStore = ConfixDocStore(RevPolicy.SEQUENTIAL)
}
