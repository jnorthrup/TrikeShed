 ...
 */
typealias JoinCondition<L, R> = (RowAccessor, RowAccessor) -> Boolean
...
fun Cursor.filter(predicate: RowPredicate): Cursor = object : Series<RowVec> {
    ...
    override val b: (Int) -> RowVec = { idx ->
        ...
        if (predicate(accessor)) {
            ...
            // The size will be adjusted by the collect operation
            0 j { _ -> null j { null } }
        }
    }
}.collect { it.size > 0 }
...
private fun Cursor.performJoin(
    ...
    val leftRows = (0 until size).map { idx ->
        this[idx].toRowAccessor() to this[idx]
    }
    val rightRows = (0 until other.size).map { idx ->
    ...
    when (joinType) {
        ...
        JoinType.FULL -> {
            ...
            for ((leftIdx, (leftAccessor, leftRow)) in leftRows.withIndex()) {
                for ((rightIdx, (rightAccessor, rightRow)) in rightRows.withIndex()) {
                    if (on(leftAccessor, rightAccessor)) {
                        joinResults.add(leftRow to rightRow)
...
suspend fun Cursor.aggregate(agg: Aggregation): Double? = when (agg) {
    is Aggregation.Sum -> {
        (0 until size).sumOf { row ->
            val accessor = row.toRowAccessor()
            agg.keyExtractor(accessor)
        }
    }
    is Aggregation.Avg -> {
        (0 until size).mapNotNull { row ->
            val accessor = row.toRowAccessor()
            agg.keyExtractor(accessor)
        }.average()
    }
...
fun gt(column: String, value: Comparable<*>): RowPredicate = { row ->
    val colValue = row[column] as? Comparable<*> ?: return@RowPredicate false
    @Suppress("UNCHECKED_CAST")
...
fun like(column: String, pattern: String): RowPredicate = { row ->
    val str = row[column]?.toString() ?: return@RowPredicate false
