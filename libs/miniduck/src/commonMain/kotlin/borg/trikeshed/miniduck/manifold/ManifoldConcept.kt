 ...
 *
 * Conceptually Axis3<Float,Float,Float> = p j (d j q), but given the test
 * API requires .p/.d/.q and a companion .unpack(), we use a small value class
 * backed by a Long (pack = 3×20-bit fixed-point) for cache efficiency.
 *
 * Pack layout (60 bits used):
 *   bits 40..59 → p (20-bit, scaled by 2^20)
 *   bits 20..39 → d
 *   bits  0..19 → q
 */
data class BudgetCoord(val p: Float, val d: Float, val q: Float) {

    /** Geometric mean of p,d,q — the "radial" energy of the concept. */
    fun energy(): Float =
        if (p <= 0f || d <= 0f || q <= 0f) 0f
        else (p.toDouble().pow(1.0 / 3.0) *
              d.toDouble().pow(1.0 / 3.0) *
              q.toDouble().pow(1.0 / 3.0)).toFloat()

    /** Pack p,d,q into a Long for dense storage or Hamming-style comparison. */
    fun pack(): Long {
        val pBits = (p.coerceIn(0f, 1f) * SCALE).toLong()
        val dBits = (d.coerceIn(0f, 1f) * SCALE).toLong()
        val qBits = (q.coerceIn(0f, 1f) * SCALE).toLong()
        return (pBits shl 40) or (dBits shl 20) or qBits
    }

    companion object {
        private const val SCALE = 0xFFFFF.toLong()  // 2^20 - 1

        fun unpack(packed: Long): BudgetCoord {
            val pBits = (packed ushr 40) and SCALE
            val dBits = (packed ushr 20) and SCALE
            val qBits = packed and SCALE
            return BudgetCoord(
                p = (pBits.toFloat() / SCALE).coerceIn(0f, 1f),
                d = (dBits.toFloat() / SCALE).coerceIn(0f, 1f),
                q = (qBits.toFloat() / SCALE).coerceIn(0f, 1f),
            )
        }
    }
}
...
class MutableNarsBag : NarsBag() {
    ...
    private val concepts: MutableList<ManifoldConcept<*>> = mutableListOf()
    ...
    override fun recallNear(centroid: Long, maxDistance: Int): Series<Any> =
        sortedSeries(concepts.filter { hamming(it.angular, centroid) <= maxDistance }
...
private fun buildBudgetTensor(concepts: List<ManifoldConcept<*>>): Tensor<Float> {
    ...
    val accessor: (Series<Int>) -> Float = { s ->
        ...
        val c = concepts[ci]
        when (di) { 0 -> c.budget.p; 1 -> c.budget.d; else -> c.budget.q }
    }
