...
class SpiritParser {
     ...
     * Parse a single byte
     */
    fun byte(name: CharSequence): SpiritParser = apply { currentOffset += 1 }

    /**
     * Parse a 16-bit unsigned short
     */
    fun ushort(name: CharSequence): SpiritParser = apply { currentOffset += 2 }

    /**
     * Parse a 32-bit unsigned int
     */
    fun uint(name: CharSequence): SpiritParser = apply { currentOffset += 4 }

    /**
     * Parse bytes with variable length
     */
    fun bytes(name: CharSequence, lengthRef: CharSequence): SpiritParser = apply {
     ...
     */
    fun optional(condition: CharSequence, block: SpiritParser.() -> Unit): SpiritParser = apply {
