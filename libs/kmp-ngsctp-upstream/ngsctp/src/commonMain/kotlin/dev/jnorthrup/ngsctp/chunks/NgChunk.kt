...
data class NgChunk_Abort(
    ...
    override val flags: ChunkFlags = ChunkFlags.empty(),
    val errorInfo: CharSequence? = null
) : NgChunk {
    override fun serialize(): ByteArray {
...
data class NgChunk_Asconf(
    ...
    sealed class AsconfParameter {
        ...
        abstract val address: CharSequence?

        data class AddIP(override val address: CharSequence) : AsconfParameter() {
            override val paramType: AsconfParamType = AsconfParamType.ADD_IP
        }

        data class DelIP(override val address: CharSequence) : AsconfParameter() {
            override val paramType: AsconfParamType = AsconfParamType.DEL_IP
        }

        data class SetPrimary(override val address: CharSequence) : AsconfParameter() {
        ...
        fun parse(buffer: ByteBuffer, length: UShort): NgChunk_Asconf {
            ...
            while (remaining >= 8) {
                ...
                if (addrLen > 0) {
                    ...
                    val address = CharSequence(addrBytes)
