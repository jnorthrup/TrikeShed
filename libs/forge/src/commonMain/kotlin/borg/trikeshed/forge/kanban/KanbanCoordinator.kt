 ...
 *
 * commonMain-safe: uses kotlinx.coroutines exclusively, no JVM imports.
 * Integrates with [KanbanKeyPool] for lease management and emits [KanbanDispatchEvent]
 * via [SharedFlow] for reactor choreography.
 *
 * The dispatch policy is an opaque string (GEPA-evolved or hand-tuned).
 * [parsePolicy] converts it to a [DispatchPolicy] that the tick loop reads.
 */
class KanbanCoordinator(
    ...
    // Reactive state for UI / reactor subscribers
    ...
    // Dispatch events for reactor fanout
    ...
    /**
     * Run one dispatch tick under the given policy.
     */
    ...
    fun startLoop(
        ...
        policy: DispatchPolicy = parsePolicy(SEED_POLICY),
    ): Job = scope.launch {
        while (isActive) {
            tick(policy)
...
// ---------------------------------------------------------------------------
// Policy -- GEPA-evolved string parsed into typed config
// ---------------------------------------------------------------------------
...
/** Parse a policy text block (key=value lines) into a [DispatchPolicy]. */
...
// ---------------------------------------------------------------------------
// Dispatch events -- sealed interface for reactor fanout
// ---------------------------------------------------------------------------
