...
@Serializable data class ReteNodeMetrics(
...
@Serializable data class AlphaNode(override val nodeId:String, override val label:String, val factType:FactType, val condition:String, val matchedFactIds:List<String>=emptyList(), override val metrics:ReteNodeMetrics=ReteNodeMetrics()) : ReteNode
@Serializable data class BetaNode(override val nodeId:String, override val label:String, val leftParentId:String, val rightParentId:String, val joinCondition:String, val matchedPairs:Int=0, override val metrics:ReteNodeMetrics=ReteNodeMetrics()) : ReteNode
@Serializable data class ProductionNode(override val nodeId:String, override val label:String, val action:ProductionAction, val parentIds:List<String>, override val metrics:ReteNodeMetrics=ReteNodeMetrics()) : ReteNode
...
@Serializable sealed interface ReteFact { val factId: String; val timestampMs: Long }
@Serializable data class CardFact(override val factId:String, val cardId:String, val columnOrdinal:Int, val assignee:String?, val priority:Int, val dependencyCount:Int, override val timestampMs:Long=platformUtils.currentTimeMillis()) : ReteFact
@Serializable data class KeyFact(override val factId:String, val keyId:String, val provider:String, val isActive:Boolean, val leasedTo:String?, val leaseExpiresAt:Long, override val timestampMs:Long=platformUtils.currentTimeMillis()) : ReteFact
@Serializable data class TickFact(override val factId:String, val tickNumber:Int, val currentlyRunning:Int, val availableKeys:Int, val queueDepth:Int, override val timestampMs:Long=platformUtils.currentTimeMillis()) : ReteFact
...
@Serializable data class CompiledReteNetwork(val alphaNodes:List<AlphaNode>, val betaNodes:List<BetaNode>, val productionNodes:List<ProductionNode>, val policy:DispatchPolicy) { val allNodes:List<ReteNode> get() = alphaNodes + betaNodes + productionNodes }

data class ReteKeySnapshot(val keyId:String, val provider:String, val status:KeyStatus, val leasedTo:String?, val leaseExpiresAt:Long)
...
class ReteEngine(val network:CompiledReteNetwork) {
    ...
    fun evaluate(cards:List<BoardCard>, keys:List<ReteKeySnapshot>, tickState:CoordinatorState): ReteSnapshot {
        ...
        val alphaResults = network.alphaNodes.map { node ->
            ...
            node.copy(matchedFactIds = matched.map { it.factId }, metrics = node.metrics.copy(
                factCount = matched.size, firingCount = node.metrics.firingCount + if (matched.isNotEmpty()) 1 else 0,
        ...
        val betaResults = network.betaNodes.map { node ->
            ...
            node.copy(matchedPairs = matchedPairs, metrics = node.metrics.copy(
                factCount = matchedPairs, firingCount = node.metrics.firingCount + if (matchedPairs > 0) 1 else 0,
        ...
        val prodResults = network.productionNodes.map { node ->
            val fired = node.parentIds.all { pid ->
                when { alphaMap.containsKey(pid) -> alphaMap[pid]!!.matchedFactIds.isNotEmpty()
                    betaMap.containsKey(pid) -> betaMap[pid]!!.matchedPairs > 0 else -> false }
            }
            node.copy(metrics = node.metrics.copy(factCount = if (fired) 1 else 0, firingCount = node.metrics.firingCount + if (fired) 1 else 0, lastFiredMs = if (fired) now else node.metrics.lastFiredMs))
        }

        val snapshot = ReteSnapshot(tickState.tickCount, now, alphaResults, betaResults, prodResults, cardFacts.size + keyFacts.size + 1, prodResults.count { it.metrics.factCount > 0 })
    ...
    private fun evaluateCardAlpha(node:AlphaNode, facts:List<CardFact>):List<ReteFact> = when {
        ...
        node.condition.contains("priority >= 2") -> facts.filter { it.priority >= 2 } else -> emptyList()
    }
    private fun evaluateKeyAlpha(node:AlphaNode, facts:List<KeyFact>, now:Long):List<ReteFact> = when {
        ...
        node.condition.contains("isActive == false") -> facts.filter { !it.isActive } else -> emptyList()
    }
    private fun evaluateTickAlpha(node:AlphaNode, fact:TickFact):List<ReteFact> = when {
        node.condition.contains("currentlyRunning <") -> { val lim = node.condition.substringAfter("<").trim().toIntOrNull() ?: 0; if (fact.currentlyRunning < lim) listOf(fact) else emptyList() }
        else -> emptyList()
    }
}

@Serializable data class ReteSnapshot(
    val tickNumber:Int, val timestampMs:Long, val alphaNodes:List<AlphaNode>, val betaNodes:List<BetaNode>,
    val productionNodes:List<ProductionNode>, val totalFacts:Int, val firedProductions:Int
) {
    fun toMermaid(): String = buildString {
        appendLine("graph TD")
        for (n in alphaNodes) appendLine("  ${n.nodeId}[${n.label}<br/>facts: ${n.matchedFactIds.size}]")
        for (n in betaNodes) { appendLine("  ${n.nodeId}{{${n.label}<br/>joined: ${n.matchedPairs}}}"); appendLine("  ${n.leftParentId} --> ${n.nodeId}"); appendLine("  ${n.rightParentId} --> ${n.nodeId}") }
        for (n in productionNodes) { val fired = n.metrics.factCount > 0; appendLine("  ${n.nodeId}[/${n.label}/]${if (fired) ":::fired" else ""}"); for (p in n.parentIds) appendLine("  $p --> ${n.nodeId}") }
