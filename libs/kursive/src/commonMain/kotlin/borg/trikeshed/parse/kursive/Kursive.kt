...
class KursiveCharSeries private constructor(
    ...
    fun rewind(checkpoint: Twin<Int>): KursiveCharSeries = apply {
        source.pos = checkpoint.a
        while (sink.size > checkpoint.b) sink.removeLast()
    }

    fun advance(count: Int = 1): KursiveCharSeries = apply { repeat(count) { source.pos++ } }
...
interface KursiveParser<T> : Join<Series<Char>, (KursiveCharSeries) -> T?> {
    val name: Series<Char> get() = a
    fun run(input: KursiveCharSeries): T? = b(input)

    operator fun invoke(input: KursiveCharSeries): T? {
...
fun <T> parser(name: Series<Char>, op: (KursiveCharSeries) -> T?): KursiveParser<T> = object : KursiveParser<T> {
    override val a: Series<Char> = name
    override val b: (KursiveCharSeries) -> T? = op
}

fun <T> parser(name: String, op: (KursiveCharSeries) -> T?): KursiveParser<T> = object : KursiveParser<T> {
    override val a: Series<Char> = name.toSeries()
    override val b: (KursiveCharSeries) -> T? = op
}

typealias KursiveStep = (KursiveCharSeries) -> Boolean
...
fun <T> KursiveParser<T>.parse(input: String): Join<T, NarsiveTrace>? = parse(KursiveCharSeries(input))

fun <T> KursiveParser<T>.parse(input: Series<Char>): Join<T, NarsiveTrace>? = parse(KursiveCharSeries(input))

fun <T> KursiveParser<T>.parse(input: KursiveCharSeries): Join<T, NarsiveTrace>? =
